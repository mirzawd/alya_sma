...
subroutine sld_cohesive_law_900(itask,ielem,iboun,igaub,delta,ctant,cnorm,tcbar,yybar)

  !-----------------------------------------------------------------------
  !****f* Solidz/sld_cohesive_law_900
  ! NAME
  !    sld_cohesive_law_900
  ! DESCRIPTION
  !    Compute the traction and tangent stiffness from linearly decreasing cohesive law
  ...
  real(rp)                   :: dcdot,dcmax,lambd
  
  logical debugging
  
  debugging = .false. !(ielem == 82) .and. (igaub == 2) 
  ...
  if (dceff>=dcmax .and. dcdot>=0.0_rp .and. dceff<=dcinf) then
     ...
     tceff = tcrit*(1.0_rp - dceff/dcinf )
     dtcdd = -1.0_rp*tcrit/dcinf
...
100 format (5(E16.8,','))
end subroutine sld_cohesive_law_900
