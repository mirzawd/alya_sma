...
subroutine sld_vect_elmope_impl(itask)
   ...
   use mod_sld_vect_assembly_elfem, only : sld_vect_element_operations_ELFEM_impl
   ...
   subds: do isubd = 1, num_subd
      packs: do ipack = 1, num_pack(isubd)
         ...
         if(     lelch(ielem) == ELFEM ) then
            ! Standard elements
            call sld_vect_element_operations_ELFEM_impl(itask,pnode,pgaus,list_elements(isubd) % packs(ipack) % l)
...
end subroutine sld_vect_elmope_impl
