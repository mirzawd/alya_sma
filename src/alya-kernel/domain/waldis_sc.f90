...
subroutine waldis_sc(itask)
  ...
  !    Adapted version of waldis(itask) for machine learning
  !    Beware: for the moment it calculates the wall distance on all boundaries not just boundaries with wall_law.
  ! OUTPUT
  !    yscab ... Boundary wall distance
  !    yscap ... Nodal wall distance, projection of yscab.
  ...
  call livinf(0_ip,'COMPUTE WALL DISTANCE TO SCALE FOR MACHINE LEARNING',0_ip)

  if( INOTMASTER ) then

     if( itask == 1 ) call domain_memory_allocate('VARIABLE SCALE DISTANCE') ! YSCAB(NBOUN) and YSCAP(NBOPO)
...
!     if ( kfl_noslw_ker /= 0_ip)  ywale = huge(1.0_rp)  ! Init
     ...
     do iboun = 1,nboun
        ...
        ! Obtain yscab = abs ( (cogbo-cogel).bonor )
        aux = 0.0_rp      
        do idime = 1,ndime
           aux   = aux + bonor(idime) * (elcog(idime)-bocog(idime)) 
        end do
        yscab(iboun) = delmu_ml * abs(aux)
...
!        if ( kfl_noslw_ker /= 0_ip)  ywale(ielem) = min(ywale(ielem),ywalb(iboun))  ! another option might have been average - perhaps take into account only boundaries type 23
     ...
     do ipoin = 1,npoin
        ibopo = lpoty(ipoin)
        if( ibopo > 0 ) yscap(ibopo) = gevec(2,ipoin)/gevec(1,ipoin)
     end do
     ...
     !
     do ibopo = 1,nbopo
        if (yscap(ibopo)<1.0e-12_rp) then
           do ipoin=1,npoin
              if (lpoty(ipoin)==ibopo) print*,'ibopo,ipoin,yscap(ibopo)',ibopo,ipoin,yscap(ibopo)
           end do
        end if
     end do
