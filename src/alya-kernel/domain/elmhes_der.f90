...
subroutine elmhes_der(&
     heslo,gphes_der,ndime,pnode,ntens,xjaci,deriv,elcod,xjaci_der)
  !-----------------------------------------------------------------------
  !****f* Domain/elmhes_der
  ! NAME
  !    elmhes_der
  ...
  real(rp),    intent(in)  :: heslo(ntens,pnode),xjaci_der(ndime,ndime,ndime,pnode)
  real(rp),    intent(out) :: gphes_der(ndime,pnode,ntens,pnode) 
!  integer(ip)              :: inode
