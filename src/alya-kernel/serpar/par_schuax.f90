...
subroutine par_schuax(&
  ...
  real(rp),    intent(in)  :: askyl(*)
  integer(ip), intent(in)  :: iskyl(*)
  integer(ip), intent(in)  :: nskyl

  real(rp),    intent(out), target :: zz(npoin_bb)

  integer(ip)              :: ii,jj,kk,col,info
  ...
  if( INOTMASTER ) then
     ...
     !
     call chosol(&
          npoi1*nbvar,nskyl,iskyl,1_ip,askyl,zi,&
          npoi1*nbvar,info)
     !
     ! zz = Abb p
     !
