...
!- parte de mod_nsi_element_operations_hh91  -- esto es solo paar P1

module mod_nsi_element_operations_hh91
...
#include "def_vector_size.inc"
   ...
   private
   public :: nsi_element_operations_hh91
...
contains
   ...
   subroutine nsi_element_operations_hh91( &
...
!!!!!!!!!!!!!!!!!!!
         ...
         do idime = 1, ndime
            do jdime = 1, ndime
               gpgve(jdime, idime) = 0
               do inode = 1, pnode
                  gpgve( jdime, idime) =  gpgve( jdime, idime) + gpcar( jdime, inode)*elvel( idime, inode)
               end do
            end do
            do igaus = 1, pgaus
               gpvel(idime, igaus) = 0
               do inode = 1, pnode
                  gpvel( idime, igaus) = gpvel( idime, igaus) + sha_aux( inode, igaus)*elvel( idime, inode)
               end do
            end do
         end do
...
!    end do
         ...
         do idime = 1, ndime
            gprhs( idime) = densi_aux*grnor_nsi*gravi_nsi(idime)
         end do
         ...
         if (ielem > 0) then
            do inode = 1, pnode
               do idime = 1, ndime
                  elrbu(idime) = 0.0
                  do igaus = 1, pgaus
                     elrbu(idime) = elrbu(idime) + gpvol*sha_aux(inode, igaus)*gprhs( idime)
                     do jdime = 1, ndime
                        elrbu(idime) = elrbu(idime) - densi_aux*gpvol*sha_aux( inode, igaus)*gpvel( jdime, igaus)*gpgve( jdime, idime) &     ! non-conservative - (u.grad) u
                        - visco_tot * gpvol * gpcar(jdime,inode) * (gpgve(jdime,idime) + gpgve(idime,jdime))         ! viscous fvins_nsi==1 leads to bulkv=1.0
                     end do
                  end do
                  do igaus = 1, pgaus
                     elrbu(idime) = elrbu(idime) - densi_aux * gpvol * sha_aux( inode, igaus)*gpvel( idime, igaus) * (gpgve( 1, 1) + gpgve( 2, 2) + gpgve( 3, 3))  ! Addit oriol
                  end do
               end do
               do idime = 1, ndime
                  ipoin = lnods_loc(inode)
...
!#endif
                  !$acc atomic update
                  rhsid(iauxi) = rhsid(iauxi) + elrbu(idime)
               end do
            end do
         end if
      end do
...
#endif
#endif
      end subroutine nsi_element_operations_hh91

      end module mod_nsi_element_operations_hh91
