...
!- eliminate dt_rho_nsi  &  mass_rho_nsi -- in mod_nsi_multi_step_fs.f90:    if(kfl_assem_nsi >39_ip 
!- put vreman inside an eliminate ker_proper
!- obtain elrbu directly
!- TO DO Revise w/ Dominick which of his improvements he thinks are worth for CPU
!- put mu_hh rho_hh in def_nastin parameters
...
module mod_nsi_element_operations_hh75
...
!hhh  
!hhh  private
!hhh  public :: nsi_element_operations_hh75
!hhh
!hhhcontains
!hhh
!hhh  !-----------------------------------------------------------------------
...
!hhh  subroutine nsi_element_operations_hh75(&
...
!hhh    use def_domain,                     only : kfl_savda
!hhh    use def_master,                     only : solve            
!hhh    use def_domain,                     only : elmda_gpvol
!hhh    use def_domain,                     only : elmda_gpcar
...
!hhh    use def_nastin,                     only : visco_nsi
...
!hhh    integer(ip)                      :: pelty,j,k,porde
...
!hhh    real(rp)                         :: gpcar(VECTOR_DIM,ndime,mnode,pgaus)              ! dN/dxi
!hhh    real(rp)                         :: gpvol(VECTOR_DIM,pgaus)                          ! w*|J|, |J|
...
!hhh    real(rp)                         :: gpgve(VECTOR_DIM,ndime,ndime,pgaus)              ! grad(u)
!hhh    real(rp)                         :: xjacm(VECTOR_DIM,ndime,ndime)
!hhh    real(rp)                         :: xjaci(VECTOR_DIM,ndime,ndime)
!hhh    real(rp)                         :: gpdet(VECTOR_DIM,pgaus)
!hhh
...
!hhh    integer(ip)                      :: jdime,jnode,idofv,jdofv
!hhh    integer(ip)                      :: ievat,jevat,iauxi,dummi
!hhh    !
!hhh    ! TEMP GABLS1
!hhh    !
!hhh#ifdef GABLS1TREF
...
!hhh    real(rp)                  :: xnutu(VECTOR_DIM,pgaus),xmile(VECTOR_DIM,pgaus)
!hhh    real(rp)                  :: G__ij(VECTOR_DIM,3,3,pgaus)
!hhh    real(rp)                  :: Aalph(VECTOR_DIM,pgaus),Bbeta(VECTOR_DIM,pgaus)
...
!hhh#ifdef OPENACCHHH
!hhh
!hhh#define FACT0X     fact0
!hhh#define FACT1X     fact1
!hhh#define FACT2X     fact2
!hhh#define FACT4X     fact4
!hhh#define FACT5X     fact5
!hhh#define FACT6X     fact6
...
!hhh
!hhh#ifdef VECTOR_SIZE_VARIABLE
!hhh#define FACT0X     fact0(1:VECTOR_DIM)
!hhh#define FACT1X     fact1(1:VECTOR_DIM)
!hhh#define FACT2X     fact2(1:VECTOR_DIM)
!hhh#define FACT4X     fact4(1:VECTOR_DIM)
!hhh#define FACT5X     fact5(1:VECTOR_DIM)
!hhh#define FACT6X     fact6(1:VECTOR_DIM)
...
!hhh#else
!hhh#define FACT0X     fact0(1:VECTOR_SIZE)
!hhh#define FACT1X     fact1(1:VECTOR_SIZE)
!hhh#define FACT2X     fact2(1:VECTOR_SIZE)
!hhh#define FACT4X     fact4(1:VECTOR_SIZE)
!hhh#define FACT5X     fact5(1:VECTOR_SIZE)
!hhh#define FACT6X     fact6(1:VECTOR_SIZE)
...
!hhh    !$acc              gpdet   ,                                          &
!hhh    !$acc              xjacm   , xjaci     , elwvi     ,                  &
!hhh    !$acc              gprhs   , elrbu                         )          &
!hhh    !$acc copyin(      visco_nsi,                                         &
...
!hhh    !--------------------------------------------------------------------
!hhh
!hhh    if( kfl_savda == 2 ) then
!hhh#ifndef OPENACCHHH
!hhh       do ivect = 1,VECTOR_DIM      
!hhh#endif
!hhh          ielem = abs(list_elements(ivect))
!hhh          if( ielem > 0 ) then
!hhh             gpvol(ivect,1:pgaus)                 = elmda_gpvol(1:pgaus,ielem)
!hhh             gpcar(ivect,1:ndime,1:mnode,1:pgaus) = elmda_gpcar(1:ndime,1:mnode,1:pgaus,ielem)
!hhh          else
!hhh             gpvol(ivect,1:pgaus)                 = 0.0_rp
!hhh             gpcar(ivect,1:ndime,1:mnode,1:pgaus) = 0.0_rp
!hhh          end if
!hhh#ifndef OPENACCHHH
!hhh       end do
!hhh#endif
...
!hhh
!hhh          do igaus = 1,pgaus
!hhh             xjacm(DEF_VECT,1,1)    =  0.0_rp
!hhh             xjacm(DEF_VECT,1,2)    =  0.0_rp
!hhh             xjacm(DEF_VECT,2,1)    =  0.0_rp
!hhh             xjacm(DEF_VECT,2,2)    =  0.0_rp
!hhh             do k = 1,pnode
!hhh                xjacm(DEF_VECT,1,1) =  xjacm(DEF_VECT,1,1) + elcod(DEF_VECT,1,k) * elmar(pelty) % deriv(1,k,igaus)
!hhh                xjacm(DEF_VECT,1,2) =  xjacm(DEF_VECT,1,2) + elcod(DEF_VECT,1,k) * elmar(pelty) % deriv(2,k,igaus)
!hhh                xjacm(DEF_VECT,2,1) =  xjacm(DEF_VECT,2,1) + elcod(DEF_VECT,2,k) * elmar(pelty) % deriv(1,k,igaus)
!hhh                xjacm(DEF_VECT,2,2) =  xjacm(DEF_VECT,2,2) + elcod(DEF_VECT,2,k) * elmar(pelty) % deriv(2,k,igaus)
!hhh             end do
!hhh             gpdet(DEF_VECT,igaus)  =  xjacm(DEF_VECT,1,1) * xjacm(DEF_VECT,2,2) - xjacm(DEF_VECT,2,1) * xjacm(DEF_VECT,1,2)
!hhh             denom                  =  1.0_rp / gpdet(DEF_VECT,igaus)
!hhh             xjaci(DEF_VECT,1,1)    =  xjacm(DEF_VECT,2,2) * denom
!hhh             xjaci(DEF_VECT,2,2)    =  xjacm(DEF_VECT,1,1) * denom
!hhh             xjaci(DEF_VECT,2,1)    = -xjacm(DEF_VECT,2,1) * denom
!hhh             xjaci(DEF_VECT,1,2)    = -xjacm(DEF_VECT,1,2) * denom
!hhh             do j = 1, pnode
!hhh                gpcar(DEF_VECT,1,j,igaus) =   xjaci(DEF_VECT,1,1) * elmar(pelty) % deriv(1,j,igaus) &
!hhh                     &                      + xjaci(DEF_VECT,2,1) * elmar(pelty) % deriv(2,j,igaus)
!hhh
!hhh                gpcar(DEF_VECT,2,j,igaus) =   xjaci(DEF_VECT,1,2) * elmar(pelty) % deriv(1,j,igaus) &
!hhh                     &                      + xjaci(DEF_VECT,2,2) * elmar(pelty) % deriv(2,j,igaus)
!hhh             end do
!hhh             gpvol(DEF_VECT,igaus) = elmar(pelty) % weigp(igaus) * gpdet(DEF_VECT,igaus)
!hhh          end do
!hhh
!hhh       else if( ndime == 3 ) then
!hhh
!hhh          do igaus = 1,pgaus
!hhh             xjacm(DEF_VECT,1:3,1:3)   = 0.0_rp
!hhh             do k = 1,pnode 
!hhh                xjacm(DEF_VECT,1,1)    =  xjacm(DEF_VECT,1,1) + elcod(DEF_VECT,1,k) * elmar(pelty) % deriv(1,k,igaus)
!hhh                xjacm(DEF_VECT,1,2)    =  xjacm(DEF_VECT,1,2) + elcod(DEF_VECT,1,k) * elmar(pelty) % deriv(2,k,igaus)
!hhh                xjacm(DEF_VECT,1,3)    =  xjacm(DEF_VECT,1,3) + elcod(DEF_VECT,1,k) * elmar(pelty) % deriv(3,k,igaus)
!hhh                xjacm(DEF_VECT,2,1)    =  xjacm(DEF_VECT,2,1) + elcod(DEF_VECT,2,k) * elmar(pelty) % deriv(1,k,igaus)
!hhh                xjacm(DEF_VECT,2,2)    =  xjacm(DEF_VECT,2,2) + elcod(DEF_VECT,2,k) * elmar(pelty) % deriv(2,k,igaus)
!hhh                xjacm(DEF_VECT,2,3)    =  xjacm(DEF_VECT,2,3) + elcod(DEF_VECT,2,k) * elmar(pelty) % deriv(3,k,igaus)
!hhh                xjacm(DEF_VECT,3,1)    =  xjacm(DEF_VECT,3,1) + elcod(DEF_VECT,3,k) * elmar(pelty) % deriv(1,k,igaus)
!hhh                xjacm(DEF_VECT,3,2)    =  xjacm(DEF_VECT,3,2) + elcod(DEF_VECT,3,k) * elmar(pelty) % deriv(2,k,igaus)
!hhh                xjacm(DEF_VECT,3,3)    =  xjacm(DEF_VECT,3,3) + elcod(DEF_VECT,3,k) * elmar(pelty) % deriv(3,k,igaus)
!hhh             end do
!hhh             t1                        =  xjacm(DEF_VECT,2,2) * xjacm(DEF_VECT,3,3) - xjacm(DEF_VECT,3,2) * xjacm(DEF_VECT,2,3)
!hhh             t2                        = -xjacm(DEF_VECT,2,1) * xjacm(DEF_VECT,3,3) + xjacm(DEF_VECT,3,1) * xjacm(DEF_VECT,2,3)
!hhh             t3                        =  xjacm(DEF_VECT,2,1) * xjacm(DEF_VECT,3,2) - xjacm(DEF_VECT,3,1) * xjacm(DEF_VECT,2,2)
!hhh             gpdet(DEF_VECT,igaus)     =  xjacm(DEF_VECT,1,1) * t1 + xjacm(DEF_VECT,1,2) * t2 + xjacm(DEF_VECT,1,3) * t3
!hhh             denom                     =  1.0_rp / gpdet(DEF_VECT,igaus)
!hhh             xjaci(DEF_VECT,1,1)       =  t1 * denom
!hhh             xjaci(DEF_VECT,2,1)       =  t2 * denom
!hhh             xjaci(DEF_VECT,3,1)       =  t3 * denom
!hhh             xjaci(DEF_VECT,2,2)       =  ( xjacm(DEF_VECT,1,1) * xjacm(DEF_VECT,3,3) - xjacm(DEF_VECT,3,1) * xjacm(DEF_VECT,1,3)) * denom
!hhh             xjaci(DEF_VECT,3,2)       =  (-xjacm(DEF_VECT,1,1) * xjacm(DEF_VECT,3,2) + xjacm(DEF_VECT,1,2) * xjacm(DEF_VECT,3,1)) * denom
!hhh             xjaci(DEF_VECT,3,3)       =  ( xjacm(DEF_VECT,1,1) * xjacm(DEF_VECT,2,2) - xjacm(DEF_VECT,2,1) * xjacm(DEF_VECT,1,2)) * denom
!hhh             xjaci(DEF_VECT,1,2)       =  (-xjacm(DEF_VECT,1,2) * xjacm(DEF_VECT,3,3) + xjacm(DEF_VECT,3,2) * xjacm(DEF_VECT,1,3)) * denom
!hhh             xjaci(DEF_VECT,1,3)       =  ( xjacm(DEF_VECT,1,2) * xjacm(DEF_VECT,2,3) - xjacm(DEF_VECT,2,2) * xjacm(DEF_VECT,1,3)) * denom
!hhh             xjaci(DEF_VECT,2,3)       =  (-xjacm(DEF_VECT,1,1) * xjacm(DEF_VECT,2,3) + xjacm(DEF_VECT,2,1) * xjacm(DEF_VECT,1,3)) * denom
!hhh
!hhh             do j = 1, pnode
!hhh                gpcar(DEF_VECT,1,j,igaus) =   xjaci(DEF_VECT,1,1) * elmar(pelty) % deriv(1,j,igaus) &
!hhh                     &                      + xjaci(DEF_VECT,2,1) * elmar(pelty) % deriv(2,j,igaus) &
!hhh                     &                      + xjaci(DEF_VECT,3,1) * elmar(pelty) % deriv(3,j,igaus)
!hhh
!hhh                gpcar(DEF_VECT,2,j,igaus) =   xjaci(DEF_VECT,1,2) * elmar(pelty) % deriv(1,j,igaus) &
!hhh                     &                      + xjaci(DEF_VECT,2,2) * elmar(pelty) % deriv(2,j,igaus) &
!hhh                     &                      + xjaci(DEF_VECT,3,2) * elmar(pelty) % deriv(3,j,igaus)
!hhh
!hhh                gpcar(DEF_VECT,3,j,igaus) =   xjaci(DEF_VECT,1,3) * elmar(pelty) % deriv(1,j,igaus) &
!hhh                     &                      + xjaci(DEF_VECT,2,3) * elmar(pelty) % deriv(2,j,igaus) &
!hhh                     &                      + xjaci(DEF_VECT,3,3) * elmar(pelty) % deriv(3,j,igaus)
!hhh             end do
!hhh             gpvol(DEF_VECT,igaus) = elmar(pelty) % weigp(igaus) * gpdet(DEF_VECT,igaus)
!hhh
!hhh!!!!!!!!!!!!!!!!!!!
!hhh             xmile(DEF_VECT,igaus) = ((  hnatu/sqrt(xjaci(DEF_VECT,1,1) * xjaci(DEF_VECT,1,1) &
...
!hhh                gpgve(DEF_VECT,jdime,idime,igaus) = gpgve(DEF_VECT,jdime,idime,igaus) &
!hhh                     + gpcar(DEF_VECT,jdime,inode,igaus) * elvel(DEF_VECT,idime,inode)
!hhh             end do
!hhh          end do
...
!hhh              G__ij(DEF_VECT,idime,jdime,igaus) = G__ij(DEF_VECT,idime,jdime,igaus) + (gpgve(DEF_VECT,idime,kdime,igaus) &
!hhh                   *gpgve(DEF_VECT,jdime,kdime,igaus)*xmile(DEF_VECT,igaus)*xmile(DEF_VECT,igaus))
!hhh           end do
!hhh        end do
!hhh     end do
!hhh    end do
...
!hhh       xnutu = 0.0_rp
!hhh    endwhere
!hhh    gpvis = gpvis + gpden * xnutu   ! creo  correcto  : gpvis(VECTOR_DIM,pgaus), xnutu(VECTOR_DIM,pgaus)
!hhh!!!!!!!
!hhh
!hhh#ifndef OPENACCHHH  
!hhh    !
...
!hhh               do igaus = 1, pgaus
!hhh                elrbu(DEF_VECT, idime, inode) = elrbu(DEF_VECT, idime, inode) + gpvol(DEF_VECT,igaus)*gpsha(DEF_VECT,inode, igaus)*gprhs(DEF_VECT, idime, igaus)
!hhh                  do jdime = 1, ndime
!hhh                     elrbu(DEF_VECT, idime, inode) = elrbu(DEF_VECT, idime, inode) &
!hhh                        - densi_aux * gpvol(DEF_VECT,igaus) * gpsha(DEF_VECT,inode,igaus) * gpvel(DEF_VECT,jdime,igaus) * gpgve(DEF_VECT,jdime,idime,igaus)     &     ! non-conservative - (u.grad) u
!hhh                        - gpvis(DEF_VECT,igaus) * gpvol(DEF_VECT,igaus) * gpcar(DEF_VECT,jdime,inode,igaus) * (gpgve(DEF_VECT,jdime,idime,igaus) + gpgve(DEF_VECT,idime,jdime,igaus))         ! viscous fvins_nsi==1 leads to bulkv=1.0
!hhh                  end do
!hhh               end do
!hhh            end do
!hhh         end do
...
!hhh               do igaus = 1, pgaus
!hhh                  elrbu(DEF_VECT, idime, inode) = elrbu(DEF_VECT, idime, inode) &
!hhh                      - densi_aux * gpvol(DEF_VECT,igaus) * gpsha(DEF_VECT,inode,igaus) * gpvel(DEF_VECT,idime,igaus) *  &
!hhh                        (gpgve(DEF_VECT,1,1,igaus) + gpgve(DEF_VECT,2,2,igaus) + gpgve(DEF_VECT,3,3,igaus) )  ! Addit oriol
!hhh               end do
!hhh            end do
!hhh         end do
!hhh
...
!hhh
!hhh            do inode = 1,pnode
!hhh                FACT1X = gpvol(DEF_VECT,igaus) * gpsha(DEF_VECT,inode,igaus)  ! ( f , v )
!hhh                do idime = 1,ndime
!hhh
...
!hhh          !
!hhh   
!hhh                 FACT2X = gpvol(DEF_VECT,igaus) / dtinv_nsi
!hhh                 do idime = 1,ndime
!hhh                    elrbp(DEF_VECT,inode) = elrbp(DEF_VECT,inode) - FACT2X * gpcar(DEF_VECT,idime,inode,igaus) * gprhs(DEF_VECT,idime,igaus) 
!hhh                 end do
!hhh    
!hhh            end do
!hhh        end do
...
!hhh#endif
!hhh    !
!hhh    ! Send only convective term to RHS
!hhh    !
!hhh
!hhh    !--------------------------------------------------------------------
!hhh    !
...
!hhh!      call nsi_corio_element_operationshh75(pnode,pgaus,gpvol,gpden,gpsha,elauu)
!hhh    end if    
!hhh#endif
!hhh
!hhh    !--------------------------------------------------------------------
...
!hhh    !$acc              elmar(pelty)%deriv ,                               &
!hhh    !$acc              elmar(pelty)%weigp, visco_nsi                      )
!hhh
!hhh#ifndef OPENACCHHH
!hhh    call cputim(timeb)
...
!hhh#endif
!hhh
!hhh  end subroutine nsi_element_operations_hh75

end module mod_nsi_element_operations_hh75
