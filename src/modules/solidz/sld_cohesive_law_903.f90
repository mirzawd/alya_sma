...
subroutine sld_cohesive_law_903(itask,ielem,iboun,igaub,delta,ctant,cnorm,tcbar,yybar)

  !-----------------------------------------------------------------------
  !****f* Solidz/sld_cohesive_law_903
  ! NAME
  !    sld_cohesive_law_903
  ! DESCRIPTION
  !    Compute the traction and tangent stiffness from modified power-law cohesive law
...
!  use def_master, only       :  ittim
  ...
  real(rp)                   :: dcdot,dcmax,lambd,power,hyper
  ...
  dcinf = parch_sld(2,pmate)
  power = parch_sld(3,pmate)
  hyper = parch_sld(4,pmate)
  lambd = parch_sld(5,pmate)
  ...
  !
  ! Effective cohesive traction based on linearly decreasing law
  !
  if (dceff>=dcmax .and. dcdot>=0.0_rp .and. dceff<=dcinf) then 
     ...
     tceff = tcrit*(1.0_rp - (dceff/dcinf)**power)*cosh(hyper*dceff/dcinf)
     dtcdd = tcrit/dcinf*((1.0_rp-(dceff/dcinf)**power)*hyper*sinh(hyper*dceff/dcinf) &
                          -power*(dceff/dcinf)**(power-1.0_rp)*cosh(hyper*dceff/dcinf))
    
  else if ((dceff<dcmax .or. dcdot<0.0_rp) .and. dceff<=dcinf .and. dcmax<=dcinf) then  ! if unloading
     !
...
100 format (5(E16.8,','))
end subroutine sld_cohesive_law_903
