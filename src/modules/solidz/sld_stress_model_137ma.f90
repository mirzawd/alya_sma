...
!> @} 
!-----------------------------------------------------------------------
subroutine sld_stress_model_137ma(&
  ...
  real(rp)                   :: a,b,af,bf,as,bs,flag_2
  ...
  real(rp)                   :: gpcau_bar(ndime,ndime),energy_a1,energy_a1_2,energy_a1_a1,energy_a1_a1_2,&
  ...
  do igaus = 1,pgaus
     ...
     if( modfi_sld(pmate) .ne. 0 ) then
        ...
        do inode = 1,pnode
          ...
          nfibe0_2(1,igaus) = nfibe0_2(2,igaus) + gpsha(inode,igaus)*elfib(2,inode)
          nfibe0_2(1,igaus) = nfibe0_2(3,igaus) + gpsha(inode,igaus)*(-elfib(3,inode))
        end do
           ...
           do jdime = 1,ndime
              do kdime = 1,ndime
                 do ldime=1,ndime
                    ...
                    term1 = 4.0_rp*(gpdet(igaus)**(-4.0_rp/3.0_rp))*energy_a1_a1*fporf(idime,jdime)*fporf(kdime,ldime)

                    term2 = (4.0_rp/3.0_rp)*energy_a1*i4f*(C4+(1.0_rp/3.0_rp)*gpcai(idime,jdime)*gpcai(kdime,ldime))
                    ...
                    term1_2 = 4.0_rp*(gpdet(igaus)**(-4.0_rp/3.0_rp))*energy_a1_a1_2*fporf_2(idime,jdime)*fporf_2(kdime,ldime)

                    term2_2 = (4.0_rp/3.0_rp)*energy_a1_2*i6f*(C4+(1.0_rp/3.0_rp)*gpcai(idime,jdime)*gpcai(kdime,ldime))
...
end subroutine sld_stress_model_137ma
