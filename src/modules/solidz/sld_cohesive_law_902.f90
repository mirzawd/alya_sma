...
subroutine sld_cohesive_law_902(itask,ielem,iboun,igaub,delta,ctant,cnorm,tcbar,yybar)

  !-----------------------------------------------------------------------
  !****f* Solidz/sld_cohesive_law_902
  ! NAME
  !    sld_cohesive_law_902
  ! DESCRIPTION
  !    Compute the traction and tangent stiffness from exponential decreasing cohesive law
  ...
  real(rp)                   :: dcdot,dcmax,lambd,fexpo
  
  logical debugging
  
  debugging = .false. !(ielem == 24) .and. (igaub == 2) 
  ...
  if (dceff>=dcmax .and. dcdot>=0.0_rp .and. dceff<=dcinf) then
     ...
     tceff = tcrit*(fexpo**(dceff/dcinf) - fexpo)/(1.0_rp - fexpo)
     dtcdd = tcrit*log(fexpo)*fexpo**(dceff/dcinf)/(1.0_rp - fexpo)/dcinf
...
100 format (5(E16.8,','))
end subroutine sld_cohesive_law_902
