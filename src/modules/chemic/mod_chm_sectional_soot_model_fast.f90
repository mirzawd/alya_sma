...
#include "def_vector_size.inc"
  ...
  ! Public subroutines
  !
  public :: assembly_soot_sources_ssm_fast
  public :: initialize_constants_ssm_fast
  public :: chm_initialization_ssm_fast
  public :: chm_validation_test_ssm_fast
  public :: chm_sectional_soot_memory_ssm_fast
  public :: soot_volume_discretization_ssm_fast
  public :: gather_soot_ssm_fast
  public :: elmpre_soot_ssm_fast


  !Non fast subroutines
  ...
  contains
    ...
    subroutine chm_validation_test_ssm_fast()
      ...
      !
      call chm_initialization_ssm_fast()

      !
      ! Reading chem1d input file
      !
      call chm_readChem1d_ssm_fast(nclas_ssm,npts,gpden,gpcon,gptem)
      ...
      !
!      call chm_calculate_soot_sources_ssm_fast(1_ip,npts,gpcon,gptem,gpden)
...
!      call footer()
    ...
    end subroutine chm_validation_test_ssm_fast

    subroutine gather_soot_ssm_fast(pnode,lnods,elsoot)
    ...
    end subroutine gather_soot_ssm_fast

    subroutine elmpre_soot_ssm_fast(pnode,pgaus,gpsha,elsoot,gpsoot)
      implicit none
      integer(ip),          intent(in)  :: pnode
...
!!DMM      end do
    ...
    end subroutine elmpre_soot_ssm_fast


    subroutine assembly_soot_sources_ssm_fast(list_elements_p, pgaus,gpcon,gptem,gpden,gprhs)
    use def_chemic,       only : kfl_yk_fw_ssm
    use def_kermod,       only : lookup_fw
      implicit none
      integer(ip),        intent(in)     :: list_elements_p(VECTOR_SIZE)
      integer(ip),        intent(in)     :: pgaus
      real(rp),           intent(in)     :: gpcon(VECTOR_SIZE,pgaus,nclas_ssm,*)
      real(rp),           intent(in)     :: gptem(VECTOR_SIZE,pgaus)
      real(rp),           intent(in)     :: gpden(VECTOR_SIZE,pgaus)
      real(rp),           intent(inout)  :: gprhs(VECTOR_SIZE,pgaus,nclas_ssm)

      ! OPTIONAL INPUT: Yk_ssm_gp_vec (species mass fractions for soot processes)
      real(rp)                          :: Yk_ssm_gp_vec(VECTOR_SIZE,pgaus,lookup_fw(kfl_yk_fw_ssm)%main_table%nvar)
      ...
      ielemone =  abs(list_elements_p(1))

      do ivect=1, VECTOR_SIZE


          ielem = abs(list_elements_p(ivect))
...
!              if (present(gpYk)) then
                 call chm_calculate_soot_sources_ssm_fast(list_elements_p,pgaus,gpcon,gptem,gpden,Yk_ssm_gp_vec)
!              else
!                 call chm_calculate_soot_sources_ssm_fast(list_elements_p,pgaus,gpcon,gptem,gpden)
!              endif
         ...
         do ivect=1, VECTOR_SIZE
          ...
          if(ivect== 1 .or. (ivect > 1 .and. ielem /= ielemone)) then
              ...
              do idsm=1,nsect_ssm
                 ...
                 gprhs(ivect,1:pgaus,iclas) = Qtot_gp_chm(ielem) % a (1:pgaus,idsm,1)
              end do
              ...
              if (kfl_spec_source_terms /= 0) then
                 do iclas=1,nspec_ssm
                    gprhs(ivect,1:pgaus,iclas) = SSTtot_gp_chm(ielem) % a (1:pgaus,iclas, 1)
                 end do
              endif

        end if
      end do
    end subroutine assembly_soot_sources_ssm_fast




   subroutine chm_sectional_soot_memory_ssm_fast()
    ...
    end subroutine chm_sectional_soot_memory_ssm_fast


    subroutine chm_calculate_soot_sources_ssm_fast(list_elements_p,pgaus,gpcon,gptem,gpden,gpYk)
      !-----------------------------------------------------------------------
      !****f* Chemic/
      ! NAME
      !
...
!      integer(ip),        intent(in)  :: ielem
      integer(ip),        intent(in)  :: list_elements_p(VECTOR_SIZE)
      integer(ip),        intent(in)  :: pgaus
      real(rp),           intent(in)  :: gpcon(VECTOR_SIZE,pgaus,nclas_ssm)     ! Mass fraction species Yk
      real(rp),           intent(in)  :: gptem(VECTOR_SIZE,pgaus)               ! Temperature
      real(rp),           intent(in)  :: gpden(VECTOR_SIZE,pgaus)               ! Density
      real(rp), optional, intent(in)  :: gpYk(VECTOR_SIZE,pgaus,n_soot_species) ! OPTIONAL INPUT: species mass fractions for soot processes

      real(rp)                        :: gpsoot(VECTOR_SIZE,pgaus,nsect_ssm)    ! Mass fraction soot species Ys
      real(rp)                        :: gpXk(VECTOR_SIZE,pgaus,n_soot_species) ! Concentration

      integer(ip)                     :: ivect, ielem, ielemone

      !New arrays
      real(rp)                        :: Qsoot_gp_chm_vec(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp)                        :: QsootDist_gp_chm_vec(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp)                        :: NDsoot_gp_chm_vec(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp)                        :: Asoot_gp_chm_vec(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp)                        :: Vsoot_gp_chm_vec(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp)                        :: Qnucl_gp_chm_vec(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp)                        :: SSTnucl_gp_chm_vec(VECTOR_SIZE,pgaus,nspec_ssm)
      real(rp)                        :: Qcoag_gp_chm_vec(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp)                        :: Qcond_gp_chm_vec(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp)                        :: SSTcond_gp_chm_vec(VECTOR_SIZE,pgaus,nspec_ssm)
      real(rp)                        :: Qsurf_gp_chm_vec(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp)                        :: SSTsurf_gp_chm_vec(VECTOR_SIZE,pgaus,nspec_ssm)

      real(rp)                        :: Qtot_gp_chm_vec(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp)                        :: SSTtot_gp_chm_vec(VECTOR_SIZE,pgaus,nspec_ssm)

    ielemone =  abs(list_elements_p(1))

     gpsoot = gpcon(1:VECTOR_SIZE,1:pgaus,idsm_first_ssm:idsm_last_ssm)

     Qsoot_gp_chm_vec     = soot_vol_frac_fast( pgaus, gpden, gpsoot )

     QsootDist_gp_chm_vec = soot_vol_frac_dist_fast( pgaus, Qsoot_gp_chm_vec )

     NDsoot_gp_chm_vec    = soot_num_den_fast( pgaus, QsootDist_gp_chm_vec)

     Asoot_gp_chm_vec     = soot_surf_area_fast( pgaus, QsootDist_gp_chm_vec )

     Vsoot_gp_chm_vec     = soot_vol_fast( pgaus, QsootDist_gp_chm_vec, NDsoot_gp_chm_vec )



    if (present(gpYk)) then
       gpXk = spec_con_fast(pgaus,nclas_ssm,gpden,gpYk)
    else
       gpXk = spec_con_fast(pgaus,nclas_ssm,gpden,gpcon)
    endif


    if (ID_NUCL > 0) &
       call chm_source_nucleation_ssm_fast(pgaus, gpXk, gptem, Qnucl_gp_chm_vec, SSTnucl_gp_chm_vec )

    if (ID_COAG > 0) &
       call chm_source_coagulation_ssm_fast(pgaus, gptem, NDsoot_gp_chm_vec ,Qcoag_gp_chm_vec)

    if (ID_COND > 0) &
       call chm_source_condensation_ssm_fast( pgaus,gpXk,gptem, NDsoot_gp_chm_vec, Qcond_gp_chm_vec, SSTcond_gp_chm_vec)

    if (ID_SURF > 0) &
        call chm_source_surfgrowth_ssm_fast( pgaus,gpXk,gptem, NDsoot_gp_chm_vec,Asoot_gp_chm_vec, &
                                          Vsoot_gp_chm_vec,Qsurf_gp_chm_vec, SSTsurf_gp_chm_vec)

    do ivect = 1, VECTOR_SIZE
         ielem = abs(list_elements_p(ivect))
         if(ivect== 1 .or. (ivect > 1 .and. ielem /= ielemone)) then
            if (ID_NUCL > 0) then
                Qnucl_gp_chm(ielem) % a(1:pgaus,1:nsect_ssm,1)   = Qnucl_gp_chm_vec(ivect,1:pgaus,1:nsect_ssm)
                SSTnucl_gp_chm(ielem) % a(1:pgaus,1:nspec_ssm,1) = SSTnucl_gp_chm_vec(ivect,1:pgaus,1:nspec_ssm)
            end if

            if (ID_COAG > 0) then
               NDsoot_gp_chm(ielem) % a(1:pgaus,1:nsect_ssm,1) = NDsoot_gp_chm_vec(ivect,1:pgaus,1:nsect_ssm)
               Qcoag_gp_chm(ielem) % a(1:pgaus,1:nsect_ssm,1)  = Qcoag_gp_chm_vec(ivect,1:pgaus,1:nsect_ssm)

            end if

            if (ID_COND > 0) then
               Qcond_gp_chm(ielem) % a(1:pgaus,1:nsect_ssm,1) = Qcond_gp_chm_vec(ivect,1:pgaus,1:nsect_ssm)
               SSTcond_gp_chm(ielem) % a(1:pgaus,1:nspec_ssm,1) = SSTcond_gp_chm_vec(ivect,1:pgaus,1:nspec_ssm)
            end if

            if (ID_SURF > 0) then
               Qsurf_gp_chm(ielem) % a(1:pgaus,1:nsect_ssm,1) = Qsurf_gp_chm_vec(ivect,1:pgaus,1:nsect_ssm)
               SSTsurf_gp_chm(ielem) % a(1:pgaus,1:nspec_ssm,1) = SSTsurf_gp_chm_vec(ivect,1:pgaus,1:nspec_ssm)
            end if
    ...
    if (ittot_chm < 10_ip) then
       Qtot_gp_chm_vec(:,1:pgaus,1:nsect_ssm) = RhoC_ssm * ( Qnucl_gp_chm_vec(:,1:pgaus,1:nsect_ssm) &
                                                    + Qcond_gp_chm_vec(:,1:pgaus,1:nsect_ssm) &
                                                    + Qsurf_gp_chm_vec(:,1:pgaus,1:nsect_ssm) )
    else
       Qtot_gp_chm_vec(:,1:pgaus,1:nsect_ssm) = RhoC_ssm * ( Qnucl_gp_chm_vec (:,1:pgaus,1:nsect_ssm) &
                                                    + Qcond_gp_chm_vec(:,1:pgaus,1:nsect_ssm) &
                                                    + Qsurf_gp_chm_vec(:,1:pgaus,1:nsect_ssm) &
                                                    + Qcoag_gp_chm_vec(:,1:pgaus,1:nsect_ssm) )
    end if

    SSTtot_gp_chm_vec(:,1:pgaus,1:nspec_ssm) = UnC * ( SSTnucl_gp_chm_vec(:,1:pgaus,1:nspec_ssm) &
                                                            + SSTcond_gp_chm_vec (:,1:pgaus,1:nspec_ssm) &
                                                        + SSTsurf_gp_chm_vec(:,1:pgaus,1:nspec_ssm) )
    ...
    do ivect = 1, VECTOR_SIZE
        ...
        if(ivect== 1 .or. (ivect > 1 .and. ielem /= ielemone)) then
            ...
            if (ittot_chm < 10_ip) then
               Qtot_gp_chm(ielem) % a (1:pgaus,1:nsect_ssm, 1) =  Qtot_gp_chm_vec(ivect,1:pgaus,1:nsect_ssm)
            else
               Qtot_gp_chm(ielem) % a (1:pgaus,1:nsect_ssm, 1) =  Qtot_gp_chm_vec(ivect,1:pgaus,1:nsect_ssm)
            end if
             SSTtot_gp_chm(ielem) % a (1:pgaus,1:nspec_ssm, 1) = SSTtot_gp_chm_vec(ivect,1:pgaus,1:nspec_ssm)
      end if
    end do


    end subroutine chm_calculate_soot_sources_ssm_fast

    subroutine chm_initialization_ssm_fast()
      !-----------------------------------------------------------------------
      !****f* Chemic/
      ! NAME
      !
    ...
    end subroutine chm_initialization_ssm_fast



    subroutine initialize_constants_ssm_fast()
    ...
    end subroutine initialize_constants_ssm_fast

    subroutine chm_readChem1d_ssm_fast(nvar, npts, Rho, Yspec, Temp)
      !-----------------------------------------------------------------------
      !****f* Chemic/
      ! NAME
      !
    ...
    end subroutine chm_readChem1d_ssm_fast


    subroutine chm_source_nucleation_ssm_fast(pgaus,gpXk,gptem,Qnucl,SSTnucl)
      !-----------------------------------------------------------------------
      !****f* Chemic/
      ! NAME
      !
      ...
      real(rp),     intent(in)  :: gpXk(VECTOR_SIZE,pgaus,n_soot_species) ! Concentration
      real(rp),     intent(in)  :: gptem(VECTOR_SIZE,pgaus)               ! Temperature
      real(rp),     intent(out) :: Qnucl(VECTOR_SIZE,pgaus,nsect_ssm)     ! Source term nucleation
      real(rp),     intent(out) :: SSTnucl(VECTOR_SIZE,pgaus,nspec_ssm)   ! Species source term from nucleation

      integer(ip)               :: k, idsm
      real(rp)                  :: Betaij_Nuc(VECTOR_SIZE,pgaus)          ! Collison frequancy factor
      real(rp)                  :: XPAH(VECTOR_SIZE,pgaus)                ! Number density of PAH

      Qnucl   = 0.0_rp
      SSTnucl = 0.0_rp

      XPAH(1:VECTOR_SIZE,1:pgaus)  = max(gpXk(1:VECTOR_SIZE,1:pgaus,SSM_SPEC_POS_A4), 0.0_rp)

      !
      ! Compute collision frequencies for nucleation
      !
      call soot_collision_freq_fast_vec(pgaus,VPAH,VPAH,gptem,Betaij_Nuc)
      ...
      if (kfl_spec_source_terms /= 0) then
         do k = 1,pgaus
            SSTnucl(:,k,soot_species_index(SSM_SPEC_POS_A4))  = -1.0_rp*(Qnucl(:,k,idsm)/(VPAH_NA))&
                * (soot_species_W(SSM_SPEC_POS_A4)*UnC)
            SSTnucl(:,k,soot_species_index(SSM_SPEC_POS_H2))  =  5.0_rp*(Qnucl(:,k,idsm)/(VPAH_NA))&
                * (soot_species_W(SSM_SPEC_POS_H2)*UnC)
         end do
      endif
      ...
      if (kfl_soot_chm == -1) then
         call soot_write_output_fast_vec('Qnucl.dat',Qnucl,pgaus,nsect_ssm)
         write(momod(modul) % lun_outpu,*)'# Nucleation module finished. '
      end if

    end subroutine chm_source_nucleation_ssm_fast

    subroutine chm_source_condensation_ssm_fast(pgaus,gpXk,gptem,NDsoot,Qcond,SSTcond)
      !-----------------------------------------------------------------------
      !****f* Chemic/
      ! NAME
      !
      ...
      real(rp),     intent(in)  :: gpXk(VECTOR_SIZE,pgaus,n_soot_species)           ! Concentration
      real(rp),     intent(in)  :: gptem(VECTOR_SIZE,pgaus)                         ! Temperature
      real(rp),     intent(in)  :: NDsoot(VECTOR_SIZE,pgaus,nsect_ssm)              ! NDSoot
      real(rp),     intent(out) :: Qcond(VECTOR_SIZE,pgaus,nsect_ssm)               ! Source term condensation
      real(rp),     intent(out) :: SSTcond(VECTOR_SIZE,pgaus,nspec_ssm)             ! Species source term from condensation

      integer(ip)               :: k, idsm
      integer(ip)               :: IST_type                             ! Intersectional dynamics type
      real(rp)                  :: Betaij_Cond(VECTOR_SIZE,pgaus)                   ! Collison frequancy factor
      real(rp)                  :: XPAH(VECTOR_SIZE,pgaus)                          ! Molar density of PAH
      real(rp)                  :: iVol                                 ! volume of section i
      real(rp)                  :: Deltarate(VECTOR_SIZE,pgaus,nsect_ssm,3)         ! Intersectional matrix
      ...
      do  idsm = 1,nsect_ssm
          ...
          iVol = Vpmean(idsm)
          call soot_collision_freq_fast_vec(pgaus,iVol,VPAH,gptem,Betaij_Cond)
      ...
      call soot_intersectdynamics_fast_vec(pgaus,IST_type,Deltarate)
      ...
      !
      if (kfl_soot_chm == -1) then
         call soot_write_output_fast_vec('Qcond.dat',Qcond,pgaus,nsect_ssm)
      end if

      if (kfl_soot_chm == -1) &
         write(momod(modul) % lun_outpu,*)'# Condensation module finished. '

    end subroutine chm_source_condensation_ssm_fast

    subroutine chm_source_coagulation_ssm_fast(pgaus,gptem,NDSoot,Qcoag)
      !-----------------------------------------------------------------------
      !****f* Chemic/
      ! NAME
      !
      ...
      real(rp),     intent(in)  :: gptem(VECTOR_SIZE,pgaus)               ! Temperature
      real(rp),     intent(in)  :: NDsoot(VECTOR_SIZE,pgaus,nsect_ssm)    ! NDSoot
      real(rp),     intent(out) :: Qcoag(VECTOR_SIZE,pgaus,nsect_ssm)     ! Source term condensation

      integer(ip)      :: i, j, k, idsm, jsect, ksect, ivol
      integer(ip)      :: CoagCutSec
      real(rp)         :: Vol_i, Vol_j, Vol_k, VTot            ! volume of section
      real(rp)         :: rcexp,ctexp
      real(rp)         :: Epsilonij, kfm(VECTOR_SIZE), Betaij_fm(VECTOR_SIZE)
      real(rp)         :: Beta_ij(VECTOR_SIZE,nsect_ssm,nsect_ssm)
      real(rp)         :: VkLow, VkUp
      real(rp)         :: split_fact, kron_del, gain(VECTOR_SIZE), loss(VECTOR_SIZE)
      integer(ip)      :: iCoagOpt
...
!--------------------------------------------------------------
      ...
      !
      if (kfl_soot_chm == -1) then
         call soot_write_output_fast_vec('Qcoag.dat',Qcoag,pgaus,nsect_ssm)
      end if

      if (kfl_soot_chm == -1) &
         write(momod(modul) % lun_outpu,*)'# Coagulation module finished. '

    end subroutine chm_source_coagulation_ssm_fast


    subroutine chm_source_surfgrowth_ssm_fast(pgaus,gpXk,gptem,NDsoot,Asoot,Vsoot,Qsurf,SSTsurf)
      !-----------------------------------------------------------------------
      !****f* Chemic/
      ! NAME
      !
      ...
      real(rp),     intent(in)   :: gpXk(VECTOR_SIZE,pgaus,n_soot_species)  ! Concentration
      real(rp),     intent(in)   :: gptem(VECTOR_SIZE,pgaus)                ! Temperature
      real(rp),     intent(in)   :: NDsoot(VECTOR_SIZE,pgaus,nsect_ssm), &  ! Soot number density
                                    Asoot(VECTOR_SIZE,pgaus,nsect_ssm),  &  ! Soot surface area density
                                    Vsoot(VECTOR_SIZE,pgaus,nsect_ssm)      ! Soot volume density
      real(rp),     intent(out)  :: Qsurf(VECTOR_SIZE,pgaus,nsect_ssm)      ! Source term condensation
      real(rp),     intent(out)  :: SSTsurf(VECTOR_SIZE,pgaus,nspec_ssm)    ! Species source term from surfce growth

      integer(ip)                :: k, idsm
      real(rp), dimension(VECTOR_SIZE,pgaus) :: conH, conH2, conOH, conO2, conH2O, conC2H2
      real(rp), dimension(VECTOR_SIZE,pgaus) :: fR1, rR1, fR2, rR2, fR3, fR4, fR5, fR6
      real(rp), dimension(VECTOR_SIZE,pgaus) :: denomD, denomC, Ydep, Ycon
      real(rp), dimension(VECTOR_SIZE,pgaus) :: Kss, sR1, sR2, sR3, sR4, sR5, sR6
      real(rp), dimension(VECTOR_SIZE,pgaus) :: RT
      ...
      real(rp)                   :: kP, kR
      real(rp), dimension(VECTOR_SIZE,pgaus) :: ProdR, ProdP,  &
                                    ProdH, ProdOH,ProdH2, ProdO2, &
                                    ProdCO, ProdH2O,ProdC2H2, ProdHR3, ProdOHR6 ! Production of gas-phase species

      real(rp), dimension(VECTOR_SIZE,pgaus,nsect_ssm,3) :: Deltarate_sg,   &  ! Intersectional matrix Surf
                                                Deltarate_oxO2, &  ! Intersectional matrix OxO2
                                                Deltarate_oxOH, &  ! Intersectional matrix OxOH
                                                Dummyrate
      real(rp), dimension(VECTOR_SIZE,pgaus,nsect_ssm)   :: surfSource, oxiSourceO2, oxiSourceOH
      ...
      call soot_surf_reac_rates_fast_vec( pgaus,IST_type_R1,'R1',VolAdd_R1,gptem,sR1, &
      ...
      call soot_surf_reac_rates_fast_vec( pgaus,IST_type_R2,'R2',VolAdd_R2,gptem,sR2, &
      ...
      call soot_surf_reac_rates_fast_vec( pgaus,IST_type_R3,'R3',VolAdd_R3,gptem,sR3, &
      ...
      call soot_surf_reac_rates_fast_vec( pgaus,IST_type_R4,'R4',VolAdd_R4,gptem,sR4, &
      ...
      call soot_surf_reac_rates_fast_vec( pgaus,IST_type_R5,'R5',VolAdd_R5,gptem,sR5, &
      ...
      IST_type_R6 = 2
       ...
       call soot_surf_reac_rates_fast_vec( pgaus,IST_type_R6,'R6',VolAdd_R6,gptem,sR6, &
    ...
    !
    if (kfl_soot_chm == -1) then
       call soot_write_output_fast_vec('Qsurf.dat',Qsurf,pgaus,nsect_ssm)
    end if

    if (kfl_soot_chm == -1) &
       write(momod(modul) % lun_outpu,*)'# Surface growth module finished. '

    end subroutine chm_source_surfgrowth_ssm_fast

    subroutine soot_volume_discretization_ssm_fast()
    ...
    end subroutine soot_volume_discretization_ssm_fast


    function soot_vol_frac_fast(pgaus, mix_den, soot_massfrac) result (soot_vol_frac)
      ...
      integer(ip), intent(in) :: pgaus
      real(rp),    intent(in) :: mix_den(VECTOR_SIZE,pgaus)
      real(rp),    intent(in) :: soot_massfrac(VECTOR_SIZE,pgaus,nsect_ssm)
      integer(ip)             :: jj
      real(rp)                :: soot_vol_frac(VECTOR_SIZE,pgaus,nsect_ssm)
...
!!      end do
 ...
 !!@     end do
          do jj=1,nsect_ssm
             soot_vol_frac(:,:,jj) = max(soot_massfrac(:,:,jj),0.0_rp)*(mix_den(:,:)/RhoC_ssm)
          end do

    end function soot_vol_frac_fast

    function soot_vol_frac_dist_fast(pgaus, sootvolfrac) result (soot_vol_frac_dist)
      ...
      integer(ip), intent(in) :: pgaus
      real(rp),    intent(in) :: sootvolfrac(VECTOR_SIZE,pgaus,nsect_ssm)
      integer(ip)             :: jj, ivol
      real(rp)                :: soot_vol_frac_dist(VECTOR_SIZE,pgaus,nsect_ssm)
...
!@@      end do
          ...
          do jj=1,nsect_ssm
             ivol = jj+1
             soot_vol_frac_dist(:,:,jj) = sootvolfrac(:,:,jj)/(Vsec(ivol) - Vsec(ivol-1))
          end do

    end function soot_vol_frac_dist_fast

    function soot_num_den_fast(pgaus, svfdist) result (soot_num_den)
      ...
      integer(ip), intent(in) :: pgaus
      real(rp),    intent(in) :: svfdist(VECTOR_SIZE,pgaus,nsect_ssm)
      integer(ip)             :: jj, ivol
      real(rp)                :: soot_num_den(VECTOR_SIZE,pgaus,nsect_ssm)
 ...
 !@     end do
          do jj=1,nsect_ssm
             ivol = jj+1
             soot_num_den(:,:,jj) = svfdist(:,:,jj)*log(Vsec(ivol)/ Vsec(ivol-1))
          end do

    end function soot_num_den_fast

    function soot_surf_area_fast(pgaus, svfdist) result (soot_surf_area)
      ...
      real(rp),    intent(in) :: svfdist(VECTOR_SIZE,pgaus,nsect_ssm)
      integer(ip)             :: jj, ivol
      real(rp)                :: soot_surf_area(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp)                :: ctexp
 ...
 !@     end do
          ...
          do jj=1,nsect_ssm
             ivol = jj+1
             soot_surf_area(:,:,jj) = pi*(6.0_rp/pi)**ctexp &
                                  * svfdist(:,:,jj) * (1.0_rp/ctexp) &
                                  * ( Vsec(ivol)**ctexp - Vsec(ivol-1)**ctexp )
          end do

    end function soot_surf_area_fast

    function soot_vol_fast(pgaus, svfdist, numden) result (soot_vol)
      ...
      integer(ip), intent(in) :: pgaus
      real(rp),    intent(in) :: svfdist(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp),    intent(in) :: numden(VECTOR_SIZE,pgaus,nsect_ssm)
      integer(ip)             :: jj, ivol
      real(rp)                :: soot_vol(VECTOR_SIZE,pgaus,nsect_ssm)
...
!@      end do
          ...
          do jj=1,nsect_ssm
             ivol = jj+1
             soot_vol(:,:,jj) = svfdist(:,:,jj) *( Vsec(ivol) - Vsec(ivol-1) ) &
                               / (numden(:,:,jj)+1.0_rp)
          end do


    end function soot_vol_fast

    function spec_con_fast(pgaus, nsp, mix_den, spec_massfrac ) result ( spec_con)
      ...
      real(rp),    intent(in) :: mix_den(VECTOR_SIZE,pgaus)
      real(rp),    intent(in) :: spec_massfrac(VECTOR_SIZE,pgaus,nsp)
      real(rp)                :: spec_con(VECTOR_SIZE,pgaus,n_soot_species)
      integer(ip)             :: ii
    ...
    end function spec_con_fast

    subroutine soot_collision_freq_fast_vec(pgaus,Vi,Vj,T,Betaij)
      !-----------------------------------------------------------------------
      !****f* Chemic/
      ! NAME
      !
      ...
      real(rp),    intent(in)  :: T(VECTOR_SIZE,pgaus)
      real(rp),    intent(out) :: Betaij(VECTOR_SIZE,pgaus)

      real(rp) :: Epsilonij
      real(rp) :: ctexp,rcexp
      real(rp) :: kfm(VECTOR_SIZE,pgaus)
    ...
    end subroutine soot_collision_freq_fast_vec

!    subroutine soot_collision_freq_fast(pgaus,Vi,Vj,T,Betaij)
!      !-----------------------------------------------------------------------
!      !****f* Chemic/
!      ! NAME
!      !
...
!                   * ( (1.0_rp/Vi + 1.0_rp/ Vj)**rcexp )
!
!    end subroutine soot_collision_freq_fast

    subroutine soot_intersectdynamics_fast_vec(pgaus,ST_type,DeltaQ)
      !-----------------------------------------------------------------------
      !****f* Chemic/
      ! NAME
      !
      ...
      real(rp),    intent(inout) :: DeltaQ(VECTOR_SIZE,pgaus,nsect_ssm,3_ip)
      integer(ip)                :: isec, ivol
      real(rp)                   :: IBout, IBin

      !
      ! Initialize output parts
      !
      DeltaQ(1:VECTOR_SIZE,1:pgaus,1:nsect_ssm,2:3) = 0.0_rp
...
!@
          ...
          do isec=1,nsect_ssm
             ...
             if (ST_type==1) then
                 ...
                 if (isec < nsect_ssm) then
                    ...
                    DeltaQ(:,:,isec,2) = DeltaQ(:,:,isec,1)/(1.0_rp-IBin)
                    ...
                    DeltaQ(:,:,isec,3) = DeltaQ(:,:,isec,1)/(1.0_rp-IBout)

                 !
                 ! Last section
                 !
                 elseif  (isec == nsect_ssm) then
                    DeltaQ(:,:,isec,2) = DeltaQ(:,:,isec,1)
                    DeltaQ(:,:,isec,3) = 0.0_rp
                 endif
    ...
    end subroutine soot_intersectdynamics_fast_vec

!    subroutine soot_intersectdynamics_fast(pgaus,ST_type,DeltaQ)
!      !-----------------------------------------------------------------------
!      !****f* Chemic/
!      ! NAME
!      !
...
!      end do
!
!    end subroutine soot_intersectdynamics_fast

    subroutine soot_surf_reac_rates_fast_vec(pgaus,ST_type,RName,VolAdd,Temp,Rcoef,&
        DeltaQ,stoiMR,ProdR,stoiMP,ProdP,NDsoot,Asoot,Vsoot)
      !-----------------------------------------------------------------------
      !
      ...
      real(rp),    intent(in)        :: Temp(VECTOR_SIZE,pgaus),Rcoef(VECTOR_SIZE,pgaus)
      real(rp),    intent(out)       :: DeltaQ(VECTOR_SIZE,pgaus,nsect_ssm,3)
      real(rp),    intent(in)        :: stoiMR,stoiMP
      real(rp),    intent(in)        :: NDsoot(VECTOR_SIZE,pgaus,nsect_ssm), &
                                        Asoot(VECTOR_SIZE,pgaus,nsect_ssm),  &
                                        Vsoot(VECTOR_SIZE,pgaus,nsect_ssm)
      real(rp),    intent(inout)     :: ProdR(VECTOR_SIZE,pgaus),ProdP(VECTOR_SIZE,pgaus)

      integer(ip)                    :: idsm, k, iStericFac,ivect
      real(rp)                       :: Xsoot, VOH, Vol_i
      real(rp), dimension(VECTOR_SIZE,pgaus)     :: TotalRate,par_a,par_b,nCarbon, &
      ...
      do idsm = 1,nsect_ssm
          ...
          if (RName == 'R6') then
             ...
             Vol_i = Vpmean(idsm)
             call soot_collision_freq_fast_vec(pgaus,Vol_i,VOH,Temp,Betaij_OH)
             RateQ(:,1:pgaus) = Betaij_OH(:,1:pgaus)*NDsoot(:,1:pgaus,idsm) / NAvo
          ...
          !
             ...
             if (iStericFac == 1) then
                ...
                do ivect = 1, VECTOR_SIZE
                    do k = 1,pgaus
                       if (StericFact(ivect,k) < 0.0_rp) then
                           StericFact(ivect,k) = 0.0_rp
                       end if
                    end do
                end do
      ...
      !
      if (ST_Type > 0) &
         call soot_intersectdynamics_fast_vec(pgaus,ST_Type,DeltaQ)
    ...
    end subroutine soot_surf_reac_rates_fast_vec

!    subroutine soot_write_output_fast(OutFname,OutVar,nrows,ncols)
!      !-----------------------------------------------------------------------
!      !****f* Chemic/
!      ! NAME
!      !
...
!
!    end subroutine soot_write_output_fast

    subroutine soot_write_output_fast_vec(OutFname,OutVar,nrows,ncols)
      !-----------------------------------------------------------------------
      !****f* Chemic/
      ! NAME
      !
      ...
      integer(ip), intent(in)  :: nrows,ncols
      character(9), intent(in) :: OutFname
      real(rp), intent(in)     :: OutVar(VECTOR_SIZE,nrows,ncols)
      integer(ip)              :: i,k,ivect
    ...
    end subroutine soot_write_output_fast_vec
...
!NON FAST ... ADDED TO BE COMPATIBLE WITH SEQUENTIAL
...
end module mod_chm_sectional_soot_model_fast
