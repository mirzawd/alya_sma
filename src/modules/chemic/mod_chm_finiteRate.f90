...
module mod_chm_finiteRate

#ifdef CANTERA
  use cantera,    only   : temperature, meanmolecularweight, viscosity, thermalConductivity, reactantstoichcoeff,&
                           productstoichcoeff, speciesIndex, setMassFractions, setState_HP, getenthalpies_rt, setState_TPX,&
  ...
  public :: chm_calc_hk_grad_Yk_others
  public :: chm_element_operations_finiteRate
...
contains
  ...
  subroutine chm_calc_hk_grad_Yk_others
    ...
    !
    use def_master,                 only : conce,tempe,therm,kfl_htran
#ifdef CANTERA
    use def_chemic,                 only : gas_chm, W_k, entha_chm
    use def_master,                 only : prthe
    use def_kermod,                 only : gasco
#endif
    use def_chemic,                 only : grad_Yk_hk,aux_nodes,grad_k_single,&
                                           grad_Yk, grad_T, grad_h
    ...
    call runend('chm_calc_hk_grad_Yk_others: Please, build with support for cantera')
#endif
  ...
  subroutine chm_element_operations_finiteRate(order,pnode,pgaus,list_elements)

    use def_master,     only : solve, cutim, rhsid
    use def_domain,     only : ntens, elmar, lnods, ltype, lorde, hnatu
    use def_chemic,     only : ncomp_chm, dt_chm, dt_rho_chm, iclaf_chm, iclai_chm, kfl_advec_chm, kfl_ellen_chm, kfl_entropy_chm,&
                               kfl_soot_chm, kfl_spray_chm, kfl_taust_chm, ADR_chm
    ...
    use mod_chm_sectional_soot_model_fast,  only : gather_soot_ssm
    use mod_chm_sectional_soot_model_fast,  only : elmpre_soot_ssm
    use mod_chm_sectional_soot_model_fast,  only : nsect_ssm
    use mod_chm_sectional_soot_model_fast,  only : idsm_first_ssm
    use mod_chm_sectional_soot_model_fast,  only : idsm_last_ssm
    ...
    integer(ip)                      :: ielem,iclas
    integer(ip)                      :: izmat,izrhs,pelty
    integer(ip)                      :: porde,plapl
    ...
    real(rp)                         :: elVtherm(mnode,nsect_ssm)
    real(rp)                         :: eltem(mnode)
    real(rp)                         :: gpvol(mgaus)                             ! |J|*w
    real(rp)                         :: gphco(mgaus)                             ! heat conductivity
    real(rp)                         :: gpsoot(mgaus,nsect_ssm)                  ! heat conductivity
    ...
    real(rp)                         :: gptem(mgaus)                             ! temperature
    real(rp)                         :: gptur(mgaus)                             ! turbulent viscosity
    real(rp)                         :: gpdivVt(mgaus,nsect_ssm)                 ! Divergence of thermophoretic velocity for
                                                                                 !  soot model
    ...
    elements: do kelem = 1_ip,size(list_elements,kind=ip)
       ...
       if( ielem > 0 ) then
          ...
          ! DMM: remove this initialization from here
          !
          gpdiv         = 0.0_rp
          gpdif         = 0.0_rp
          gpdit         = 0.0_rp
          ...
          call chm_elmgac_finiteRate(&
                   pnode,lnods(1:pnode,ielem),elcod,elcon(1:pnode,1:nspec_chm),eltem,&
                   elvel,grad_Yk_hk_node,grad_h_node,grad_Yk_node,entha_node,elgrad_T)

          if (kfl_soot_chm /= 0) then
             call gather_soot_ssm(pnode,lnods(1:pnode,ielem),elcon(1:pnode,idsm_first_ssm:idsm_last_ssm))
             call chm_thermophoretic_gather(pnode,nsect_ssm,idsm_first_ssm,lnods(1:pnode,ielem),elVtherm(1:pnode,1:nsect_ssm))
          end if
          !
          ...
          call chm_elmpre_finiteRate(&
                   pnode,pgaus,elcon(1:pnode,1:nclas_chm),elvel,eltem,elmar(pelty)%shape,&
                   gpcon(1:pgaus,1:nclas_chm,:),gpvel,gptem,grad_Yk_hk_node,grad_h_node,grad_Yk_hk_gp, &
                   grad_h_gp,grad_Yk_node,entha_node,elgrad_T,hk_grad_Yk_gp,grad_T_gp)

          if (kfl_soot_chm /= 0) then
             call elmpre_soot_ssm(pnode,pgaus,elmar(pelty)%shape,&
                                  elcon(1:pnode,idsm_first_ssm:idsm_last_ssm),gpcon(1:pgaus,idsm_first_ssm:idsm_last_ssm,1))

             call chm_thermophoretic_elmpre(pnode,pgaus,nsect_ssm,elmar(pelty)%shape,&
                                  elVtherm(1:pnode,1:nsect_ssm),&
                                  gpdivVt(1:pgaus,1:nsect_ssm))
          endif
          !
          ! Compute molecular transport
          ! TODO: Probably should be after entropy viscosity
          !
          call chm_elmprc_finiteRate(pgaus,gphco,gpsph,gpdif,gpcon,gptem,gpden)
          ...
          !
          if (kfl_soot_chm > 0 ) then
             call assembly_soot_sources_ssm(ielem,pgaus,gpcon,gptem,gpden,gprhs)
          end if

          !
          ! Calculate RHS with thermophoresis
          !
          if (kfl_soot_chm /= 0) then
            gprhs(1:pgaus,idsm_first_ssm:idsm_last_ssm) = gprhs(1:pgaus,idsm_first_ssm:idsm_last_ssm) &
                                                        -gpdivVt(1:pgaus,1:nsect_ssm)
          ...
          ASSEMBLY_ICLAS: do iclas = iclai_chm,iclaf_chm
             ...
             if( order == ELEMENT_ASSEMBLY ) then
                ...
                call ADR_projections_and_sgs_assembly(&
                     ielem,pnode,pgaus,elcod,elmar(pelty)%shape,gpcar,gphes,gpvol,chale,ADR_chm(iclas),&
                     cutim,gpden,gpvel,gpdif(1:pgaus,iclas),gpgrd,gprea,gprhs(1:pgaus,iclas),gpcon(1:pgaus,iclas:iclas,:),&
                     elcon(1:pnode,iclas:iclas))
  ...
  end subroutine chm_element_operations_finiteRate
   ...
   subroutine chm_updtcc_finiteRate(dtmin)
     ...
     use def_chemic,         only : ncomp_chm, iclaf_chm, iclai_chm, kfl_advec_chm, kfl_ellen_chm, kfl_spray_chm, ADR_chm
     use mod_ker_proper,     only : ker_proper
     ...
     if( INOTMASTER ) then
        ...
        do ielem = 1,nelem
           ...
           !
           call chm_elmgac_finiteRate(&
                    pnode,lnods(1:pnode,ielem),elcod,elcon(1:pnode,1:nclas_chm),eltem,&
           ...
           call chm_elmpre_finiteRate(&
                    pnode,pgaus,elcon(1:pnode,1:nclas_chm),elvel,eltem,elmar(pelty)%shape,&
                    gpcon(1:pgaus,1:nclas_chm,:),gpvel,gptem,dummr_node,dumr2_node,dummr_gp,dumr2_gp, &
                    dumr3_node,dumr4_node,dumr5_node,dumr3_gp,dumr4_gp)


           call chm_elmprc_finiteRate(&
                    pgaus,gphco,gpsph,gpdif,gpcon,gptem,gpden)
  ...
  subroutine chm_elmgac_finiteRate( &
                 pnode,lnods,elcod,elcon,eltem,elvel,grad_Yk_hk_node,grad_h_node, &
                 grad_Yk_node,entha_node,elgrad_T)
     !------------------------------------------------------------------------
     ...
     real(rp),    intent(out) :: elgrad_T(pnode,ndime)
...
#ifndef CANTERA
    ! Early exit until alternatives are added
    call runend('chm_elmgac_finiteRate: Please, build with support for cantera')
#endif
     ...
     if (kfl_htran == 0) then
        ...
        do inode=1,pnode
           ...
           elgrad_T(inode,1:ndime) = grad_T(1:ndime,ipoin)
           elcod(1:ndime,inode)       = coord(1:ndime,ipoin)
        end do
     end if
    ...
    do inode = 1,pnode
        ...
        call setMassFractions(gas_chm,conce(ipoin,1:nspec_chm,1))
        call setState_HP(gas_chm,therm(ipoin,1),prthe(1))
        eltem(inode) = temperature(gas_chm)
     end do
#endif
  ...
  subroutine chm_elmpre_finiteRate(&
                 pnode,pgaus,elcon,elvel,eltem,gpsha,&
                 gpcon,gpvel,gptem,grad_Yk_hk_node,grad_h_node,grad_Yk_hk_gp,grad_h_gp, &
                 grad_Yk_node,entha_node,elgrad_T,hk_grad_Yk_gp,grad_T_gp)
     ...
     real(rp),     intent(in)  :: elgrad_T(pnode,ndime)
        ...
        do igaus = 1,pgaus
           do inode = 1,pnode
              grad_T_gp(igaus,1:ndime) = grad_T_gp(igaus,1:ndime) &
                                              + gpsha(inode,igaus) * elgrad_T(inode,1:ndime)
           end do
        end do
     end if
  ...
  subroutine chm_elmprc_finiteRate(&
    ...
    !***
    !-----------------------------------------------------------------------
    use def_chemic, only      : Le_k
    use def_chemic, only      : kfl_transport_chm
...
#ifdef CANTERA
    integer(ip)               :: igaus
    real(rp)                  :: dens, gplocdiftest(nclas_chm)

    external                  :: cantera_mix_diff_coeffs
#endif
...
#ifndef CANTERA
    ! Early exit until alternatives are added
    call runend('chm_elmprc_finiteRate: Please, build with support for cantera')
    if (.false.) print *, gpcon, gptem ! avoid compiler warning (unused-dummy-argument)
#endif
    ...
    !
    ! Lewis number unity and constant
    !
    if ( kfl_transport_chm == 1_ip .or. kfl_transport_chm == 2_ip ) then
       do iclas = 1,nspec_chm
          gpdif(1:pgaus,iclas) = diff(1:pgaus) / Le_k(iclas)
       enddo
    elseif ( kfl_transport_chm == 3_ip ) then
       !
       ! Mixtured Averaged Diffusion - coefficients from CANTERA
       !
#ifdef CANTERA
       do igaus = 1, pgaus
          call cantera_mix_diff_coeffs(gptem(igaus),prthe(1),gpcon(igaus,1:nspec_chm),gplocdiftest(1:nclas_chm),dens)
          gpdif(igaus,1:nspec_chm) = gplocdiftest(1:nclas_chm) * dens
       enddo
#endif
    endif

    !
    ! Diffusion coefficent for soot
    !
    if (kfl_soot_chm > 0) then
        Dsoot = 1.0e-6_rp
        do iclas = nspec_chm+1,nclas_chm
           gpdif(1:pgaus,iclas) = Dsoot*gpden(1:pgaus)
        end do
    end if
  ...
  subroutine chm_getProp_finiteRate
    ...
    !-----------------------------------------------------------------------
    use def_kintyp, only      : ip,rp
    use def_master, only      : condu_gp,visco_gp,wmean_gp,&
                                sphec_gp_ht,sphec_gp_lt,&
                                sphec_gp

    use def_domain, only      : nelem,ltype,nnode,&
                                ngaus,llapl,lorde,ltopo,elmar,lnods
    use def_chemic, only      : kfl_transport_chm,hk_gp
#ifdef CANTERA
    use def_chemic, only      : gas_chm,coeff_cp_k,W_k,kfl_model_chm,kfl_solve_cond_CMC_chm,kfl_soot_chm
    use def_master, only      : prthe,tempe,conce,sphec
    use mod_chm_sectional_soot_model_fast, only: nsect_ssm, RhoC_ssm, gasCoupling_ssm
    use def_kermod, only      : gasco
    use def_domain, only      : nboun,ltypb,lnodb
#endif
    ...
    real(rp)    :: cploc(6,2),aux_h
    real(rp)    :: sootsum(mgaus),wgasold(mgaus),wgasnew(mgaus)
    real(rp)    :: soot_sp_coef(5)
    !
    ! Soot part
    !
    integer(ip) :: idsm,j
...
#ifndef CANTERA
    ! Early exit until alternatives are added
    call runend('chm_getProp_finiteRate: Please, build with support for cantera')
#endif
    ...
    if (INOTMASTER) then
       ...
       elements: do ielem = 1,nelem
          ...
          if( pelty > 0 ) then
              ...
              !
              if (kfl_transport_chm /= 4_ip) then
                 ...
                 do igaus = 1,pgaus
                    ...
                    if (kfl_model_chm == 4 .and. kfl_solve_cond_CMC_chm == 1) then  ! Compute cp for the mixture for CMC model
                       do ivalu = 1,6
                    ...
                    !
                    ! Soot model influences density (added through molecular weight) and
                    ! and C_p coefficients
                    !
                    if (kfl_soot_chm > 0) then
                       sootsum = 0.0_rp
                       wgasold = 0.0_rp
                       ...
                       if (gasCoupling_ssm == 1) then
                          soot_sp_coef(1)  = -4.322_rp
                          soot_sp_coef(2)  = 0.054280_rp
                          soot_sp_coef(3)  = -3.98E-05_rp
                          soot_sp_coef(4)  = 1.321E-08_rp
                          soot_sp_coef(5)  = -1.613E-12_rp

                          do j= 1,5
                             do idsm= 1,nsect_ssm
                                sphec_gp_lt(ielem) % a(igaus,j,1) = sphec_gp_lt(ielem) % a(igaus,j,1)+ &
                                                          soot_sp_coef(j)*gpcon(igaus,nspec_chm+idsm)
                             end do
                          end do

                          do j= 1,5
                             do idsm= 1,nsect_ssm
                                sphec_gp_ht(ielem) % a(igaus,j,1) = sphec_gp_ht(ielem) % a(igaus,j,1)+ &
                                                          soot_sp_coef(j)*gpcon(igaus,nspec_chm+idsm)
                             end do
                          end do

                          !
                          ! Correct the molecular weight to introduce
                          ! soot effects on density through the EoS
                          !
                          do idsm= 1,nsect_ssm
                             sootsum(igaus) = sootsum(igaus) + gpcon(igaus,nspec_chm+idsm)
                          end do

                          wgasold(igaus) =  meanmolecularweight(gas_chm) / 1000.0_rp
                          wgasnew(igaus) = 1000.0_rp / meanmolecularweight(gas_chm) + &
                                     ((prthe(1)/(8.3144626_rp*gptem(igaus)))*(sootsum(igaus)/(RhoC_ssm)))

                          wgasnew(igaus) = 1.0_rp / wgasnew(igaus)

                          wmean_gp(ielem) % a(igaus,1,1) = wgasnew(igaus)
                       end if
...
#endif
              elseif (kfl_transport_chm == 4_ip) then
                 !
  ...
  subroutine get_rates_pfa(temp,press,y_k,gen,r_mat)
    ...
    !-----------------------------------------------------------------------
      ...
      integer(ip)                       :: i , j , k, m
      double precision                  :: div, addp, addc
...
#ifndef CANTERA
    ! Early exit until alternatives are added
    call runend('get_rates_pfa: Please, build with support for cantera')
    if (.false.) print *, press, temp, y_k ! avoid compiler warning (unused-dummy-argument)
#endif
...
! Initialize Some Stuff
  ...
  subroutine get_reduced_reaction_rates(r_mat,key_spec,react)
    ...
    !-----------------------------------------------------------------------
      ...
      double precision                            :: maxv, div
      integer(ip)                                 :: spec(nspec_chm)
      integer,intent(inout)                       :: react(nreac_chm)
      integer(ip), intent(in)                     :: key_spec(kfl_key_chm)

#ifdef CANTERA
      external                                    :: cantera_divide
      external                                    :: cantera_log
#endif
      external                                    :: runend

#ifndef CANTERA
    ! Early exit until alternatives are added
    call runend('get_reduced_reaction_rates: Please, build with support for cantera')
#endif
      ...
      do i=1_ip,size(key_spec,kind=ip)
...
#ifndef CANTERA
    ! Early exit until alternatives are added
    call runend('get_prog_var: Please, build with support for cantera')
#endif
...
#endif
      !prog_var = y_k(index_key(1)) * 4.0/44.0 + y_k(index_key(2)) * 2.0/18.0 +  y_k(index_key(3)) * 1/28.0 + y_k(index_key(4)) * 0.5/2.0
      !prog_var = y_k(index_key(1)) * 0.125 + y_k(index_key(2)) * 0.5 +  y_k(index_key(3)) * 0.125 + y_k(index_key(4)) * 0.25
      prog_var = y_k(index_key(1)) * 0.1_rp + y_k(index_key(2)) * 0.4_rp +  y_k(index_key(3)) * 0.10_rp + y_k(index_key(4)) * 0.2_rp
  ...
  subroutine get_correlation(y_k_old,y_k_new,tempe_old,tempe_new,corr)
    ...
    !-----------------------------------------------------------------------
      ...
      double precision                            :: max_corr_coeff
#ifdef CANTERA
      character(len=:), allocatable               :: key_species_trimmed
#endif
      external                                    :: runend
...
#ifndef CANTERA
    ! Early exit until alternatives are added
    call runend('get_correlation: Please, build with support for cantera')
#endif
  ...
  subroutine get_mixture_fraction(y_k,z)
    ...
    !-----------------------------------------------------------------------
      ...
      implicit none
      double precision                            :: b_chm
      double precision, intent(in)                :: y_k(nspec_chm)
      double precision, intent(inout)             :: z
#ifdef CANTERA
      double precision                            :: dummi_h, dummi_c, dummi_o
...
#ifndef CANTERA
    ! Early exit until alternatives are added
    call runend('get_mixture_fraction: Please, build with support for cantera')
    if (.false.) print *, y_k ! avoid compiler warning (unused-dummy-argument)
#endif
      ...
      b_chm = 0.0_rp
        ...
        b_chm = ( 2.0_rp* dummi_c/12.0107_rp )  + ( 0.5_rp * dummi_h/1.00794_rp ) - dummi_o/15.999_rp
#endif
  ...
  subroutine chm_IntegrateSource_finiteRate(dt)
    ...
    use def_master,                 only : conce,dtinv
...
#ifdef CANTERA
    use def_master,                 only : tempe,prthe
    use def_chemic,                 only : kfl_int_chm
    ...
    integer(ip)                         :: ipoin ,gen, corr, dummi, ii, jj, nrow
    ...
    external                            :: cantera_odepim_ref_adaptive
    external                            :: cantera_dac_integrate
...
#ifndef CANTERA
    ! Early exit until alternatives are added
    call runend('chm_IntegrateSource_finiteRate: Please, build with support for cantera')
    ...
    if (INOTMASTER) then
       ...
       if ( kfl_pfa_chm == -1_ip ) then
            ...
            do ipoin=1,npoin
               ...
               if (T_0(ipoin) > temp_lim) then
                   if (kfl_int_chm < 0_ip) then
                      call cantera_integrate(T_0(ipoin),prthe(1),Y_k_0(ipoin,:),dt)
                   else if (kfl_int_chm == 1_ip) then
                      call cantera_odepim_ref_adaptive(nspec_chm,T_0(ipoin),prthe(1),Y_k_0(ipoin,:),dt)
                   else if (kfl_int_chm == 2_ip) then
#ifdef PYJAC
                      call cantera_integrate_pyjac(T_0(ipoin),prthe(1),Y_k_0(ipoin,:),dt)
#else
                      call runend('In chm_IntegrateSource_finiteRate: Complie with PYJAC')
#endif
                   else if (kfl_int_chm == 3_ip) then
#ifdef PYJAC
                      call cantera_integrate_rosenbrock(T_0(ipoin),prthe(1),Y_k_0(ipoin,:),dt)
#else
                      call runend('In chm_IntegrateSource_finiteRate: Complie with PYJAC')
#endif
                  end if
       ...
       !
          ...
          if(ittot_chm == 1_ip) then
             ...
             call memory_alloca(mem_modul(1:2,modul),'RETVA', 'chm_IntegrateSource_finiteRate', retva, table_fw % main_table % nvar)
             ...
             do ipoin=1,npoin
                ...
                if (retva(1) /= 1.0_rp) then
                   ...
                   call get_rates_pfa(T_0(ipoin),prthe(1),Y_k_0(ipoin,:),gen,r_mat)
                   call get_reduced_reaction_rates(r_mat,key_spec,React_ind(ipoin,:))
                   table_fw % main_table % tab(irow,2:(nreac_chm+1)) = real(React_ind(ipoin,:),rp)
       ...
       do ipoin=1,npoin
          src_chm(ipoin,1:nspec_chm)  = (Y_k_0(ipoin,1:nspec_chm) - conce(ipoin,1:nspec_chm,1)) * dtinv * prope_tmp(ipoin)
       end do
  ...
  subroutine chm_heatRelease_finiteRate
    use def_domain,                 only : elmar, nelem, ltype, nnode, ngaus, lnods
    use def_chemic,                 only : hrr_chm, hrr_int_chm
    use mod_communications_global,  only : PAR_SUM
#ifdef CANTERA
    use def_master,                 only : conce, tempe, prthe
...
#ifndef CANTERA
    ! Early exit until alternatives are added
    call runend('chm_heatRelease_finiteRate: Please, build with support for cantera')
#endif
