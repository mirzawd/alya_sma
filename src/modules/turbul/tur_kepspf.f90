...
subroutine tur_kepspf(&
  ...
  ! DESCRIPTION
  !
  !    Compute coefficient of k-eps-phi-f model
  !
  !    D.R. Laurence, J.C. Uribe and S.V. Utyuzhnikov, A Robust 
  !    Formulation of the v2-f Model. Flow, Turb. & Comb. 73, 
  !    169-185, 2004.
  !
  !    Let M(u) =  rho*Du/Dt - div[ (mu+mut/su) grad(u) ]
  !    1. M(k)   = Pk - rho*eps
  !    2. M(e)   = (Ce1*Pk-Ce2*rho*eps)/T
  !    3. M(phi) = rho*f -Pk*phi/k + 2*mut/(sk*k)*grad(phi)*grad(k)
  !    4. L^2*Lapl(f) - f = 1/T*(C1-1)*(phi-2/3) -C2*Pk(rho*k) 
  !                         -2*nu/k*grad(phi)*grad(k) -nu*Lapl(phi)
  !
  !    - T   = max[k/eps,6*sqrt(nu/eps)]
  !    - mut = rho*Cmu*phi*k*T
  !    - Eqn 1: eps substituted using defintion of T and mut 
  !    - Eqn 4: divided by L^2
  !
  ! OUTPUT 
  !    GPREA .......... r 
  !    GPDIF .......... k 
  ...
  real(rp),    intent(in)    :: gplap,gpcar(ndime,pnode)
  ...
  real(rp)                   :: Ce1,Ce2,Ce3,nu,g,T,sk
  real(rp)                   :: CL,Cn,c1,c2,L,f,oneT,oneL2
  real(rp)                   :: onReT,fmu,Ry,maxfg
