...
subroutine tur_sstkom_der(&
  ...
  real(rp),    intent(inout) :: drea_dtur(nturb_tur,pnode)
  real(rp),    intent(inout) :: dsou_dtur(nturb_tur,pnode)
  real(rp),    intent(inout) :: dsou_dvel(ndime,pnode)
...
!  real(rp)                 :: P
  ...
  if( ( gpome > xsmal ) .or. ( kfl_clipp_tur > 1 ) ) then
    ...
    do inode = 1,pnode
      ...
      dc2_dk(inode) = 0.0_rp
      dc2_dw(inode) = -500.0_rp * dgpome(inode) * gpvis / ( gpden * gpwal * gpwal * gpome**2.0_rp )
