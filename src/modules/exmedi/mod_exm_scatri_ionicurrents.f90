...
!!#############################
module mod_exm_scatri_ionicurrents
  implicit none
  private

  public :: exm_scatri_ionicurrents
...
!! State variables declaration
    ...
    beta_K1 = (-1.509_rp*safe_exp(0.0002_rp*(V*1000.0_rp - E_K*1000.0_rp + 100.0_rp)) + &
                safe_exp(0.5886_rp*(V*1000.0_rp - E_K*1000.0_rp - 10.0_rp)))/ &
              (1.0_rp + safe_exp(0.4547_rp*(V*1000.0_rp - E_K*1000.0_rp)))
    x_K1_inf = Alpha_K1/(Alpha_K1 + beta_K1)

    vicel_exm(1, ipoin) = g_Na*(m**3.0_rp)*h*j*(V - E_Na)
    vicel_exm(2, ipoin) = (g_CaL*4.0_rp*V*F**2.0_rp)*(Ca_i*safe_exp(2.0_rp*V*F/(Rcons*Temp)) - &
                          0.341_rp*Ca_o)*d*f1*f2*f_Ca/(Rcons*Temp*(safe_exp(2.0_rp*V*F/(Rcons*Temp)) - 1.0_rp))
    vicel_exm(3, ipoin) = g_to*r*q*(V - E_K)
    vicel_exm(4, ipoin) = g_Kr*sqrt(K_o/5.4_rp)*xr1*xr2*(V - E_K)
    vicel_exm(5, ipoin) = g_Ks*Xs**2.0_rp*(1.0_rp + 0.6_rp/(1.0_rp + (3.8_rp*1e-5_rp/Ca_i)**1.4_rp))*(V - E_Ks)
    vicel_exm(6, ipoin) = g_K1*x_K1_inf*sqrt(K_o/5.4_rp)*(V - E_K)
    vicel_exm(7, ipoin) = g_f*Xf*(V - E_f)
    vicel_exm(8, ipoin) = ((P_NaK*K_o*Na_i/(K_o + K_mk))/(Na_i + K_mNa))/(1.0_rp + 0.1245_rp* &
                          safe_exp(-0.1_rp*V*F/(Rcons*Temp)) + 0.353_rp*safe_exp(-V*F/(Rcons*Temp)))
    vicel_exm(9, ipoin) = K_NaCa*(safe_exp(Gamma*V*F/(Rcons*Temp))*Na_i**3.0_rp*Ca_o - &
                          safe_exp((Gamma - 1.0_rp)*V*F/(Rcons*Temp))*Na_o**3.0_rp*Ca_i*Alpha)/ &
                          ((Km_Na_i**3.0_rp + Na_o**3.0_rp)*(Km_Ca + Ca_o)*(1.0_rp + K_sat* &
                          safe_exp((Gamma - 1.0_rp)*V*F/(Rcons*Temp))))
    vicel_exm(10, ipoin) = ((a_rel*(Ca_SR**2.0_rp)/(b_rel**2.0_rp + Ca_SR**2.0_rp)) + c_rel)*d*g*(0.0556_rp)
    vicel_exm(11, ipoin) = Vmax_up/(1.0_rp + K_up**2.0_rp/Ca_i**2.0_rp)
    vicel_exm(12, ipoin) = V_leak*(Ca_SR - Ca_i)
    vicel_exm(13, ipoin) = g_pCa*Ca_i/(Ca_i + K_pCa)
...
!! Applying Runge-Kutta
  ...
  !! K2
    K2h = dtime*func_hA(tt + dtime/2.0_rp, V + K1V/2.0_rp, h + K1h/2.0_rp)
    K2j = dtime*func_jA(tt + dtime/2.0_rp, V + K1V/2.0_rp, j + K1j/2.0_rp)
    K2m = dtime*func_mA(tt + dtime/2.0_rp, V + K1V/2.0_rp, m + K1m/2.0_rp)
    K2d = dtime*func_dA(tt + dtime/2.0_rp, V + K1V/2.0_rp, d + K1d/2.0_rp)
    K2f_Ca = dtime*func_fCaA(tt + dtime/2.0_rp, f_Ca + K1f_Ca/2.0_rp, Ca_i + K1Ca_i/2.0_rp, V + K1V/2.0_rp)
    K2f1 = dtime*func_f1A(tt + dtime/2.0_rp, V + K1V/2.0_rp, f1 + K1f1/2.0_rp, Ca_i + K1f1/2.0_rp, df1)
    K2f2 = dtime*func_f2A(tt + dtime/2.0_rp, V + K1V/2.0_rp, f2 + K1f2/2.0_rp)
    K2r = dtime*func_rA(tt + dtime/2.0_rp, V + K1V/2.0_rp, r + K1r/2.0_rp)
    K2q = dtime*func_qA(tt + dtime/2.0_rp, V + K1V/2.0_rp, q + K1q/2.0_rp)
    K2xr1 = dtime*func_xr1A(tt + dtime/2.0_rp, V + K1V/2.0_rp, xr1 + K1xr1/2.0_rp)
    K2xr2 = dtime*func_xr2A(tt + dtime/2.0_rp, V + K1V/2.0_rp, xr2 + K1xr2/2.0_rp)
    K2Xs = dtime*func_XsA(tt + dtime/2.0_rp, V + K1V/2.0_rp, xs + K1Xs/2.0_rp)
    K2Xf = dtime*func_XfA(tt + dtime/2.0_rp, V + K1V/2.0_rp, xf + K1Xf/2.0_rp)
    K2g = dtime*func_gA(tt + dtime/2.0_rp, Ca_i + K1Ca_i/2.0_rp, g + K1g/2.0_rp, V + K1V/2.0_rp)
    K2Ca_i = dtime*func_CaiA(tt+dtime/2.0_rp,  Ca_i+K1Ca_i/2.0_rp,  Ca_SR+K1Ca_SR/2.0_rp, d+K1d/2.0_rp,  &
      g+K1g/2.0_rp,  V+K1V/2.0_rp,  Na_i+K1Na_i/2.0_rp,  f1+K1f1/2.0_rp  ,f2+K1f2/2.0_rp  ,f_Ca+K1f_Ca/2.0_rp)
    K2Ca_SR = dtime*func_CaSRA(tt + dtime/2.0_rp, Ca_i + K1Ca_i/2.0_rp, Ca_SR + K1Ca_SR/2.0_rp, &
      d + K1d/2.0_rp, g + K1g/2.0_rp)
    K2Na_i = dtime*func_NaiA(tt+dtime/2.0_rp,  m+K1m/2.0_rp,  h+K1h/2.0_rp,  j+K1j/2.0_rp,  &
      V+K1V/2.0_rp,  Na_i+K1Na_i/2.0_rp,  Ca_i+K1Ca_i/2.0_rp)
    K2V = dtime*func_VA(tt + dtime/2.0_rp, V + K1V/2.0_rp, r + K1r/2.0_rp, q + K1q/2.0_rp, &
      xr1 + K1xr1/2.0_rp, xr2 + K1xr2/2.0_rp, xs + K1Xs/2.0_rp, Ca_i + K1Ca_i/2.0_rp, &
      d + K1d/2.0_rp, f1 + K1f1/2.0_rp, f2 + K1f2/2.0_rp, f_Ca + K1f_Ca/2.0_rp, &
      Na_i + K1Na_i/2.0_rp, m + K1m/2.0_rp, h + K1h/2.0_rp, j + K1j/2.0_rp, xf + K1Xf/2.0_rp)

  !! K3
    K3h = dtime*func_hA(tt + dtime/2.0_rp, V + K2V/2.0_rp, h + K2h/2.0_rp)
    K3j = dtime*func_jA(tt + dtime/2.0_rp, V + K2V/2.0_rp, j + K2j/2.0_rp)
    K3m = dtime*func_mA(tt + dtime/2.0_rp, V + K2V/2.0_rp, m + K2m/2.0_rp)
    K3d = dtime*func_dA(tt + dtime/2.0_rp, V + K2V/2.0_rp, d + K2d/2.0_rp)
    K3f_Ca = dtime*func_fCaA(tt + dtime/2.0_rp, f_Ca + K2f_Ca/2.0_rp, Ca_i + K2Ca_i/2.0_rp, V + K2V/2.0_rp)
    K3f1 = dtime*func_f1A(tt + dtime/2.0_rp, V + K2V/2.0_rp, f1 + K2f1/2.0_rp, Ca_i + K2f1/2.0_rp, df1)
    K3f2 = dtime*func_f2A(tt + dtime/2.0_rp, V + K2V/2.0_rp, f2 + K2f2/2.0_rp)
    K3r = dtime*func_rA(tt + dtime/2.0_rp, V + K2V/2.0_rp, r + K2r/2.0_rp)
    K3q = dtime*func_qA(tt + dtime/2.0_rp, V + K2V/2.0_rp, q + K2q/2.0_rp)
    K3xr1 = dtime*func_xr1A(tt + dtime/2.0_rp, V + K2V/2.0_rp, xr1 + K2xr1/2.0_rp)
    K3xr2 = dtime*func_xr2A(tt + dtime/2.0_rp, V + K2V/2.0_rp, xr2 + K2xr2/2.0_rp)
    K3Xs = dtime*func_XsA(tt + dtime/2.0_rp, V + K2V/2.0_rp, xs + K2Xs/2.0_rp)
    K3Xf = dtime*func_XfA(tt + dtime/2.0_rp, V + K2V/2.0_rp, xf + K2Xf/2.0_rp)
    K3g = dtime*func_gA(tt + dtime/2.0_rp, Ca_i + K2Ca_i/2.0_rp, g + K2g/2.0_rp, V + K2V/2.0_rp)
    K3Ca_i =  dtime*func_CaiA(tt+dtime/2.0_rp,  Ca_i+K2Ca_i/2.0_rp,  Ca_SR+K2Ca_SR/2.0_rp,  d+K2d/2.0_rp,  &
      g+K2g/2.0_rp,  V+K2V/2.0_rp,  Na_i+K2Na_i/2.0_rp,  f1+K2f1/2.0_rp  ,f2+K2f2/2.0_rp  ,f_Ca+K2f_Ca/2.0_rp)
    K3Ca_SR = dtime*func_CaSRA(tt + dtime/2.0_rp, Ca_i + K2Ca_i/2.0_rp, Ca_SR + K2Ca_SR/2.0_rp, d + K2d/2.0_rp, &
      g + K2g/2.0_rp)
    K3Na_i =  dtime*func_NaiA(tt+dtime/2.0_rp,  m+K2m/2.0_rp,  h+K2h/2.0_rp,  j+K2j/2.0_rp,  V+K2V/2.0_rp,  &
      Na_i+K2Na_i/2.0_rp,  Ca_i+K2Ca_i/2.0_rp)
    K3V =     dtime*func_VA(tt+dtime/2.0_rp,  V+K2V/2.0_rp,  r+K2r/2.0_rp,  q+K2q/2.0_rp,  xr1+K2xr1/2.0_rp,  &
      xr2+K2xr2/2.0_rp,  xs+K2Xs/2.0_rp,  Ca_i+K2Ca_i/2.0_rp,  d+K2d/2.0_rp,  f1+K2f1/2.0_rp,  f2+K2f2/2.0_rp,  &
      f_Ca+K2f_Ca/2.0_rp,  Na_i+K2Na_i/2.0_rp,  m+K2m/2.0_rp,  h+K2h/2.0_rp,  j+K2j/2.0_rp  ,xf+K2Xf/2.0_rp)
  ...
  !! K4
    ...
    K4Ca_i =  dtime*func_CaiA(tt+dtime,  Ca_i+K3Ca_i,  Ca_SR+K3Ca_SR,  d+K3d,  g+K3g,  V+K3V,  &
      Na_i+K3Na_i,  f1+K3f1  ,f2+K3f2  ,f_Ca+K3f_Ca)
    ...
    K4V =     dtime*func_VA(tt+dtime,  V+K3V,  r+K3r,  q+K3q,  xr1+K3xr1,  xr2+K3xr2,  xs+K3Xs,  &
      Ca_i+K3Ca_i,  d+K3d,  f1+K3f1,  f2+K3f2,  f_Ca+K3f_Ca,  Na_i+K3Na_i,  m+K3m,  h+K3h,  j+K3j  ,xf+K3Xf)
...
!! L-type Ca2+ current, I_CaL
    I_CaL = (g_CaL*4.0_rp*V*F**2.0_rp)*(Ca_i*safe_exp(2.0_rp*V*F/(Rcons*Temp)) - &
                              0.341_rp*Ca_o)*d*f1*f2*f_Ca/(Rcons*Temp*(safe_exp(2.0_rp*V*F/(Rcons*Temp)) - 1.0_rp))
!! Na+/Ca2+ exchanger current, I_NaCa
    I_NaCa = K_NaCa*(safe_exp(Gamma*V*F/(Rcons*Temp))*Na_i**3.0_rp*Ca_o - &
        safe_exp((Gamma - 1.0_rp)*V*F/(Rcons*Temp))*Na_o**3.0_rp*Ca_i*Alpha)/((Km_Na_i**3.0_rp + &
                          Na_o**3.0_rp)*(Km_Ca + Ca_o)*(1.0_rp + K_sat*safe_exp((Gamma - 1.0_rp)*V*F/(Rcons*Temp))))
...
!! Ca2+ dynamics
    !Atrial
    I_rel = ((a_rel*(Ca_SR**2.0_rp)/(b_rel**2.0_rp + Ca_SR**2.0_rp)) + c_rel)*d*g*(0.0556_rp)

    I_up = Vmax_up/(1.0_rp + K_up**2.0_rp/Ca_i**2.0_rp)
    I_leak = V_leak*(Ca_SR - Ca_i)

    Ca_ibufc = 1.0_rp/(1.0_rp + Buf_c*K_Buf_c/(Ca_i + K_Buf_c)**2.0_rp)
    func_CaiA = Ca_ibufc*(I_leak - I_up + I_rel - (I_CaL + I_bCa + I_pCa - 2.0_rp*I_NaCa)*C_m/(2.0_rp*V_c*F*1.0e-18_rp))
  end function
...
!#####################################################################################
  real(rp) function func_dA(tt, V, d)
    ...
    !I_CaL, d gate
    !Atrial
    d_inf = 1.0_rp/(1.0_rp + safe_exp(-(V*1000.0_rp + 5.986_rp)/7.0_rp))
...
!! I_CaL, f1 gate
    !Atrial
    f1_inf = 1.0_rp/(1.0_rp + safe_exp((V*1000.0_rp + 25.226_rp)/3.0_rp))

    if (df1 > 0.0_rp) then
        taw_f1 = ((1102.5_rp*(safe_exp(-(((V*1000.0_rp + 27.0_rp)**2.0_rp)/15.0_rp)**2.0_rp)) + &
    200.0_rp/(1.0_rp+safe_exp((13.0_rp-V*1000.0_rp)/10.0_rp))+180.0_rp/(1.0_rp+safe_exp((30.0_rp+V*1000.0_rp)/10.0_rp))+20.0_rp)*(1.0_rp+1433.0_rp*(Ca_i-50.0_rp*1e-6_rp)))/1000.0_rp
    else
        taw_f1 = ((1102.58_rp*(safe_exp(-(((V*1000.0_rp + 27.0_rp)**2.0_rp)/15.0_rp)**2.0_rp)) + &
    200.0_rp/(1.0_rp+safe_exp((13.0_rp-V*1000.0_rp)/10.0_rp))+180.0_rp/(1.0_rp+safe_exp((30.0_rp+V*1000.0_rp)/10.0_rp))+20.0_rp))/1000.0_rp
    end if
...
!! I_CaL, f2 gate
    !Atrial
    f2_inf = (0.67_rp/(1.0_rp + safe_exp((V*1000.0_rp + 31.226_rp)/4.0_rp))) + 0.33_rp
    taw_f2 = ((600.0_rp*(safe_exp((-(V*1000.0_rp + 25.0_rp)**2.0_rp)/170.0_rp)) + &
              31.0_rp/(1.0_rp+safe_exp((25.0_rp-V*1000.0_rp)/10.0_rp))+16.0_rp/&
              (1.0_rp+safe_exp((30.0_rp+V*1000.0_rp)/10.0_rp)))*2.0_rp)/1000.0_rp
...
!! I_Na, h gate
    ...
    if (V < -0.04_rp) then
      ...
      beta_h = 2.7_rp*safe_exp(0.079_rp*V*1000.0_rp) + &
              3.1_rp*(10.0_rp**5.0_rp)*safe_exp(0.3485_rp*V*1000.0_rp)
    else
      Alpha_h = 0.0_rp
...
!! I_Na, j gate
    ...
    if (V < -0.04_rp) then
      Alpha_j = (-25428.0_rp*safe_exp(0.2444_rp*V*1000.0_rp) - 6.948_rp*(1e-6_rp)* &
              safe_exp(-0.04391_rp*V*1000.0_rp))*(V*1000.0_rp + 37.78_rp)/(1.0_rp + safe_exp(0.311_rp*(V*1000.0_rp + 79.23_rp)))
      beta_j = 0.02424_rp*safe_exp(-0.01052_rp*V*1000.0_rp)/(1.0_rp + &
                safe_exp(-0.1378_rp*(V*1000.0_rp + 40.14_rp)))
    else
      Alpha_j = 0.0_rp
...
!! I_Na, m gate
    ...
    beta_m = 0.1_rp/(1.0_rp + safe_exp((V*1000.0_rp + 35.0_rp)/5.0_rp)) + &
              0.1_rp/(1.0_rp + safe_exp((V*1000.0_rp - 50.0_rp)/200.0_rp))
...
!! Na+/K+ pump current, I_NaK
    I_NaK = ((P_NaK*K_o*Na_i/(K_o + K_mk))/(Na_i + K_mNa))/(1.0_rp + &
                0.1245_rp*safe_exp(-0.1_rp*V*F/(Rcons*Temp)) + 0.353_rp*safe_exp(-V*F/(Rcons*Temp)))

!! Na+/Ca2+ exchanger current, I_NaCa
    I_NaCa = K_NaCa*(safe_exp(Gamma*V*F/(Rcons*Temp))*(Na_i**3.0_rp)*Ca_o - &
        safe_exp((Gamma - 1.0_rp)*V*F/(Rcons*Temp))*(Na_o**3.0_rp)*Ca_i*Alpha)/ &
        ((Km_Na_i**3.0_rp + Na_o**3.0_rp)*(Km_Ca + Ca_o)*(1.0_rp + K_sat*safe_exp((Gamma - 1.0_rp)*V*F/(Rcons*Temp))))

!! sodium dynamics
    func_NaiA = -C_m*(I_Na + I_bNa + 3.0_rp*I_NaK + 3.0_rp*I_NaCa)/(F*V_c*1e-18_rp)
  end function
...
!! I_to, q gate
    ...
    taw_q = (39.102_rp/(0.57_rp*safe_exp(-0.08_rp*(V*1000.0_rp + 44.0_rp)) + &
                        0.065_rp*safe_exp(0.1_rp*(V*1000.0_rp + 45.93_rp))) + 6.06_rp)/1000.0_rp
...
!#####################################################################################
  real(rp) function func_rA(tt, V, r)
    ...
    taw_r = (14.405516_rp/(1.037_rp*safe_exp(0.09_rp*(V*1000.0_rp + 30.61_rp)) + &
                            0.369_rp*safe_exp(-0.12_rp*(V*1000.0_rp + 23.84_rp))) + 2.75352_rp)/1000.0_rp
...
!! Inward rectifier K+ current, I_K1
    Alpha_K1 = 3.91_rp/(1.0_rp + safe_exp(0.5942_rp*(V*1000.0_rp - E_K*1000.0_rp - 200.0_rp)))
    beta_K1 = (-1.509_rp*safe_exp(0.0002_rp*(V*1000.0_rp - E_K*1000.0_rp + 100.0_rp)) + &
                safe_exp(0.5886_rp*(V*1000.0_rp - E_K*1000.0_rp - 10.0_rp)))/(1.0_rp + &
                safe_exp(0.4547_rp*(V*1000.0_rp - E_K*1000.0_rp)))
    x_K1_inf = Alpha_K1/(Alpha_K1 + beta_K1)
...
!! Na+/K+ pump current, I_NaK
    I_NaK = ((P_NaK*K_o*Na_i/(K_o + K_mk))/(Na_i + K_mNa))/(1.0_rp + 0.1245_rp* &
            safe_exp(-0.1_rp*V*F/(Rcons*Temp)) + 0.353_rp*safe_exp(-V*F/(Rcons*Temp))); 
!! Na+/Ca2+ exchanger current, I_NaCa
    I_NaCa = K_NaCa*(safe_exp(Gamma*V*F/(Rcons*Temp))*Na_i**3.0_rp*Ca_o - &
              safe_exp((Gamma - 1.0_rp)*V*F/(Rcons*Temp))*Na_o**3.0_rp*Ca_i*Alpha)/ &
...
!#####################################################################################
  real(rp) function func_xr1A(tt, V, xr1)
    ...
    V_half = 1000.0_rp*(-(Rcons*Temp/(F*Qcons))*log(((1.0_rp + Ca_o/2.6_rp)**4.0_rp)/ &
                          (L_0*(1.0_rp + Ca_o/0.58_rp)**4.0_rp)) - 0.019_rp)
...
end module mod_exm_scatri_ionicurrents
